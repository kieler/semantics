...
//    @Test
    ...
    public def void spin_default_invar(SCCharts scc, TestModelData modelData) {
    ...
    @Test
    // Failed in modulo-3int-8bit-par
    public def void spin_default_ltl(SCCharts scc, TestModelData modelData) {
        mustNotHaveModelProperties = #["unbounded-int", "ctl-specs"]
        ignoreInvar = true
        ignoreLtl= false
        ignoreCtl = true
...
//    @Test
    // Failed in modulo-3int-8bit-par
    public def void spin_bfs_invar(SCCharts scc, TestModelData modelData) {
        mustNotHaveModelProperties = #["unbounded-int", "ctl-specs"]
...
//    @Test
    // Failed in modulo-3int-8bit-par
    public def void spin_bfs_ltl(SCCharts scc, TestModelData modelData) {
        mustNotHaveModelProperties = #["unbounded-int", "ctl-specs"]
        customSpinCommands = #["-bfs"]
        ignoreInvar = true
        ignoreLtl= false
...
//                recordStatistics()
    ...
    override onVerificationPropertyChanged(VerificationPropertyChanged event) {
        ...
        // Set failedToProve flag for statistics if there was an exception
        if(property.status == VerificationPropertyStatus.EXCEPTION) {
            System.err.println("onVerificationPropertyChanged with EXCEPTION "+property.cause)
            failedToProve = true
            
            // Check which exception the property got to determine if the test timed out.
            // These are the exceptions that interrupt when waiting for a response (InterruptedException)
            // or that just kill the process communications (IOException).
            val cause = property.cause
            if(cause instanceof InterruptedException || cause instanceof IOException) {
                failedToProve = false
                timedOut = true
            }
        }
    ...
    public def void recordStatistics() {
        if(currentVerificationProperty === null || !isInitialWarmupDone) {
            return
        }
