        ...
        if (state.outgoingTransitions.size == 1 && !state.hierarchical) {
            ...
            if (transition.immediate2) {
                if (transition.trigger == null && transition.effects.nullOrEmpty) {
                    ...
                    targetState.mapParents(transition.mappedParents); // NEW - redirect mapping information
                    transition.unmapAll; // NEW - remove mapping information
                    ...
                    targetState.mapParents(state.mappedParents); // NEW - redirect mapping information
                    state.unmapAll; // NEW - remove mapping information
                }
            }
        }
    ...
    def void transformSurfaceDepth(State state, Region targetRootRegion) {
        if (state.outgoingTransitions.size > 0 && state.type == StateType::NORMAL
            ...
            for (transition : immediateTransitions) {                
                //-- New - manual copy to copy mappings 
                ...
                copyMapping.entries.forEach[
                    it.value.mapParents(it.key.mappedParents); //NEW - mapping information 
                ]
            ...
            for (transition : orderedTransitionList) {
                ...
                if (!(transition.isImmediate) && !tickBoundaryInserted) {
                    ...
                    depthState.mapParents(surfaceState.mappedParents); //NEW - mapping information 
                    val connect = previousState.createImmediateTransitionTo(depthState)
                    connect.mapParents(transition.mappedParents); //NEW - mapping information 
                ...
                if (currentState == null) {
                    ...
                    currentState.mapParents(surfaceState.mappedParents); //NEW - mapping information 

                    // Connect
                    val connect = previousState.createTransitionTo(currentState)
                    connect.mapParents(surfaceState.mappedParents); //NEW - mapping information 
            ...
            T2tmp.mapParents(previousState.mappedParents); //NEW - mapping information 
                    ...
                    if (!K1.outgoingTransitions.filter(e|e != T1).nullOrEmpty &&
                        ...
                        if (TK1s.size > 0 && TK2s.size > 0) {
                            ...
                            if ((TK1.targetState == TK2.targetState) &&
                                ...
                                stateAfterDepth.mapParents(K2.mappedParents); // NEW - redirect mapping information     
                                K2.unmapAll; // NEW - remove mapping information                          
                                for (transition : K2.outgoingTransitions) {
                                    transition.targetState.incomingTransitions.remove(transition)
                                    stateAfterDepth.mapParents(transition.mappedParents); // NEW - redirect mapping information
                                    transition.unmapAll; // NEW - remove mapping information
                                    transition.getAllContents(true).forEach[it.unmapAll];
                                }
                                done = false
                                T2tmp = t
                            }
