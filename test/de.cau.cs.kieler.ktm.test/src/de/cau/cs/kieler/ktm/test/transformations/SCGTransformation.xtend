...
class SCGTransformation {
    ...
    def SCGraph transformSCG(Region rootRegion) {
        ...
        rootRegion.mapChild(sCGraph); //NEW - transformation mapping information
        rootRegion.rootState.mapChild(sCGraph); //NEW - transformation mapping information
        ...
        for (valuedObject : rootRegion.rootState.valuedObjects) {
            ...
            valuedObjectSCG.mapParent(valuedObject); //NEW - transformation mapping information
        }
...
//                                            e.effects.nullOrEmpty].size == 1) &&
        (state.
            outgoingTransitions.filter[e|e.isImmediate && e.effects.nullOrEmpty].size == 2) &&
    ...
    def SCGraph trimExitNodes(SCGraph sCGraph) {
        ...
        for (exitNode : superfluousExitNodes.immutableCopy) {
            val links = sCGraph.eAllContents.filter(typeof(ControlFlow)).filter(e|e.target == exitNode).toList
            for (link : links) {
            ...
            if (exitNode.next != null) {
                ...
                exitNode.next.mapParents(link.mappedParents); //NEW - redirect transformation mapping information
                link.unmapAll; //NEW - remove transformation mapping information
            }
    ...
    def SCGraph trimConditioanlNodes(SCGraph sCGraph) {
        ...
        for (conditionalNode : superfluousConditionalNodes.immutableCopy) {
            val links = sCGraph.eAllContents.filter(typeof(ControlFlow)).filter(e|e.target == conditionalNode).toList
            for (link : links) {
            ...
            if (conditionalNode.getElse != null) {
                ...
                (conditionalNode.getElse.target as Conditional).getThen.mapParents(linkThen.mappedParents); //NEW - redirect transformation mapping information
                (conditionalNode.getElse.target as Conditional).getElse.mapParents(linkElse.mappedParents); //NEW - redirect transformation mapping information
                
                linkThen.unmapAll; //NEW - remove transformation mapping information
                linkElse.unmapAll; //NEW - remove transformation mapping information
            }
            sCGraph.nodes.remove(conditionalNode)
            
            conditionalNode.getElse.target.mapParents(conditionalNode.mappedParents); //NEW - redirect transformation mapping information
            conditionalNode.unmapAll; //NEW - remove transformation mapping information
        }
        sCGraph
    }
    ...
    def void transformSCGGenerateNodes(Region region, SCGraph sCGraph) {
        ...
        region.mapChild(entry);//NEW - transformation mapping information
        val exit = sCGraph.addExit
        region.mapChild(exit); //NEW - transformation mapping information
        region.map(entry)
        entry.setExit(exit)
        exit.map(region)
    ...
    def void transformSCGGenerateNodes(State state, SCGraph sCGraph) {
        ...
        if (state.pause) {
            ...
            state.mapChild(depth);  //NEW - transformation mapping information
            surface.setDepth(depth)
            surface.map(state)
            state.map(surface)
        } else if (state.assignment) {
            val assignment = sCGraph.addAssignment
            state.mapChild(assignment);  //NEW - transformation mapping information
            ...
            transition.effects.get(0).mapChild(assignment);  //NEW - transformation mapping information
            ...
            (transition.effects.get(0) as de.cau.cs.kieler.sccharts.Assignment).expression.mapChild(assignment.assignment); //NEW - transformation mapping information
        } else if (state.conditional) {
            val conditional = sCGraph.addConditional
            state.mapChild(conditional);  //NEW - transformation mapping information
            state.outgoingTransitions.forEach[it.mapChild(conditional)];//NEW - transformation mapping information
            ...
            transition.trigger.mapChild(conditional.condition); //NEW - transformation mapping information
        } else if (state.fork) {
            val fork = sCGraph.addFork
            state.mapChild(fork);  //NEW - transformation mapping information
            val join = sCGraph.addJoin
            state.mapChild(join);  //NEW - transformation mapping information
            fork.setJoin(join)
            state.map(fork)
            join.map(state)
            ...
            val exit = sCGraph.addExit
            state.mapChild(exit);  //NEW - transformation mapping information
            state.map(exit)
        }
    }
    ...
    def void transformSCGConnectNodes(Region region, SCGraph sCGraph) {
        ...
        initialState.outgoingTransitions.get(0).mapChild(controlFlowInitial); //NEW - transformation mapping information
    ...
    def void transformSCGConnectNodes(State state, SCGraph sCGraph) {
        ...
        if (state.pause) {
            ...
            if (otherNode != null) {
                ...
                state.outgoingTransitions.get(0).mapChild(controlFlow);  //NEW - transformation mapping information
            ...
            if (otherNodeThen != null) {
                ...
                transitionThen.mapChild(controlFlowThen);  //NEW - transformation mapping information
                conditional.setThen(controlFlowThen)
            }
            if (otherNodeElse != null) {
                val controlFlowElse = otherNodeElse.createControlFlow
                transitionElse.mapChild(controlFlowElse);  //NEW - transformation mapping information
            ...
            for (region : state.regions) {
                ...
                if (otherNodeEntry != null) {
                    ...
                    state.mapChild(controlFlowEntry);  //NEW - transformation mapping information
                ...
                if (otherNodeExit != null) {
                    ...
                    state.mapChild(controlFlowFinal);  //NEW - transformation mapping information
            ...
            if (state.outgoingTransitions.size > 0) {
                ...
                if (otherNodeTermination != null) {
                    ...
                    termination.mapChild(controlFlowTermination);  //NEW - transformation mapping information
                    
                    // Add another assignment if the termination has an effect.
                    if (!termination.effects.nullOrEmpty) {
                        val assignment = sCGraph.addAssignment
                        termination.mapChild(assignment);  //NEW - transformation mapping information
                        termination.effects.get(0).mapChild(assignment);  //NEW - transformation mapping information
                        ...
                        assignment.setValuedObject(sCChartAssignment.valuedObject.getSCGValuedObject)
                        assignment.setAssignment(sCChartAssignment.expression.convertToSCGExpression)                        
                        (termination.effects.get(0) as de.cau.cs.kieler.sccharts.Assignment).expression.mapChild(assignment.assignment); //NEW - transformation mapping information
                        assignment.setNext(controlFlowTermination)
                        join.setNext(assignment.createControlFlow)
                        termination.mapChild(join.next);  //NEW - transformation mapping information                  
                    } else {
                ...
                state.mapChild(controlFlow);  //NEW - transformation mapping information
