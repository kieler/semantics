...
// new include for execution on aeon needed (segmentation fault at function
// inet_ntoa otherwise!
#include <arpa/inet.h>
...
#define SCADEOK       "OK\n" //!< SCADE ACK (with or without trailing "\n")
#define SCADEOKWIN2K  "OK"
#define DEBUG 0              //!< turn DEBUG mode on (1) or off (0)
#define DEFAULTGUIPORT 2020  //!< us this port if not any defined in cfg file
#define RAILCONFIGFILE "railway.cnf" //!< config file name
#define LOGFILENAME "railway.log"    //!< log file name


//some makro definitions for the log / debug text output

#define DEBUGTEXT1(PARA1) {{ \
if ((DEBUG)||(!QUIETMODE)) {  \
	if (LOGFILEOUTPUT) {  \
	   char dest[1024]; \
	   sprintf(dest, PARA1); \
...
#define DEBUGTEXT2(PARA1,PARA2) {{ \
if ((DEBUG)||(!QUIETMODE)) {  \
	if (LOGFILEOUTPUT) {  \
	   char dest[1024]; \
	   sprintf(dest, PARA1, PARA2); \
	   print2log(dest);  \
	} else { \
	   printf(PARA1, PARA2);  \
} } } }

#define DEBUGTEXT3(PARA1,PARA2, PARA3) {{ \
if ((DEBUG)||(!QUIETMODE)) {  \
	if (LOGFILEOUTPUT) {  \
	   char dest[1024]; \
	   sprintf(dest, PARA1, PARA2, PARA3); \
	   print2log(dest);  \
	} else { \
	   printf(PARA1, PARA2, PARA3);  \
} } } }

#define DEBUGTEXT4(PARA1,PARA2, PARA3, PARA4) {{ \
if ((DEBUG)||(!QUIETMODE)) {  \
	if (LOGFILEOUTPUT) {  \
	   char dest[1024]; \
	   sprintf(dest, PARA1, PARA2, PARA3, PARA4); \
	   print2log(dest);  \
	} else { \
	   printf(PARA1, PARA2, PARA3, PARA4);  \
} } } }
...
//!
int getIniValue(char* keyName, char* valueData, char* buffer);
...
//!				connect to
//!
extern void initSimulation(int port);
