...
class Variable implements Cloneable {
    ...
    @Accessors
    private val DataPoolEntry poolEntry
    @Accessors
    private val DataPool pool    
    /**
     * The variable type.
     */
    @Accessors
     ...
     */
    @Accessors
    private var Object value //= poolEntry.getTypedValue(null)
...
//        DataPoolView.instance.userValues.put(poolEntry.name, )
    }
...
//    }
    ...
    public def Object toggledPresentState() {
        if(value instanceof Boolean) {
            ...
            throw new Exception("Can't toggle present state of variable "+poolEntry.name+" because it is neither a boolean nor an integer.")
        }
    }
...
//    }
    ...
    override Variable clone() {
        val v = new Variable(poolEntry, pool)
        v.type = this.type
     ...
     */
    override toString() {
        return "variable "+poolEntry.name
    }
}
