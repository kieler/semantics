...
public class RxtxSerialConnection extends ConnectionProtocol {
	...
	/**
	 * Retrieves all available serial ports and returns a list of
	 * port names.
	...
	public static String[] getSerialPorts() {
		...
		while (ports.hasMoreElements()) {
			CommPortIdentifier identifier = ports.nextElement();
	...
	/*	try {
            ...
            return "Serial port " + device + " set up for " + BAUD_RATE + " baud, 8 data bits, 1 stop bit, no parity check.";
        } catch (Exception e) {
            throw new CommunicationException("Error during initialization of port "
            		+ device + ": " + e.getMessage());
        }*/
