...
public class LineScanner extends DepthFirstAdapter {
	...
	public void inAInitialTransition(AInitialTransition node) {
		// System.out.println("line " + node.getEdge().getLine() + ": \"->" +
		// node.getTargetState() + "\"");
		// key = "transition:" +
		// stack.peek().substring(stack.peek().indexOf(":")+1) + "-0->" +
		// node.getTargetState().getText().toUpperCase();
	...
	public void inAOtherTransition(AOtherTransition node) {
		...
		// }
		// System.out.println(key);
		// key = "transition:" + node.getSourceState().getText().toUpperCase() +
		// "-%->" + node.getTargetState().getText().toUpperCase();
		// stack.push(key);
	...
	/*
	 * @Override public void outAOtherTransition(AOtherTransition node) { key =
	 * stack.pop(); //System.out.println("out transition, key = " + key);
	 * 
	 * //determine if none, one or two % /*int count = -1; if
	 * (!key.contains("%")){ count = 0; } else{ if
	 * (!key.substring(key.indexOf('%')+1).contains("%")){ count = 1; } else{
	 * count = 2; } } //if 0, add key if (count == 0){ if
	 * (sem2line.containsKey(key)){ for (int line : sem2line.get(key)){
	 * line2line.get(line).add(node.getEdge().getLine()); } } else{
	 * System.out.println("unknown key: " + key); } } //if 1, replace % by 0 and
	 * add key if (count == 1){ key = key.replaceAll("%", "0"); if
	 * (sem2line.containsKey(key)){ for (int line : sem2line.get(key)){
	 * line2line.get(line).add(node.getEdge().getLine()); } } else{
	 * System.out.println("unknown key: " + key); } } //if 2, try prio key and 0
	 * key if (count == 2){ String key2 = key; key = key.replaceAll("%", "");
	 * key2 = key2.replaceAll("%[1-9]%", "0"); if (sem2line.containsKey(key)){
	 * for (int line : sem2line.get(key)){
	 * line2line.get(line).add(node.getEdge().getLine()); } } else{
	 * System.out.println("unknown key: " + key + ", now trying " + key2); if
	 * (sem2line.containsKey(key2)){ for (int line : sem2line.get(key2)){
	 * line2line.get(line).add(node.getEdge().getLine()); } } else{
	 * System.out.println("unknown key: " + key2); } } }
	 * 
	 * /* if (key.contains("%")){ key = key.replaceAll("%", "0"); } if
	 * (sem2line.containsKey(key)){ for (int line : sem2line.get(key)){
	 * line2line.get(line).add(node.getEdge().getLine()); } } else{
	 * System.out.println("unknown key: " + key); }
	 *//*
		 * //System.out.println(key); super.outAOtherTransition(node); }
		 */

	/*
	 * @Override public void caseANormalTranstype(ANormalTranstype node) {
	 * String key = stack.pop(); stack.push("normaltermination:" +
