...
public abstract class Identifer {
    ...
    public String getValidName() {
        if ((name.equals("_COUNT")) || (name.equals("R0"))
                || (name.equals("R1")) || (name.equals("R2"))
                || (name.equals("R3")) || (name.equals("TICK"))
    ...
    protected String encode(int width) {
        ...
        while (result.length() < width)
            result = "0" + result;
        assert result.length() == width : "Opcode length for " + name
                + " should be " + width + " but is " + result.length() + "!";
