...
public class KrepProtocol extends CommunicationProtocol {
    ...
    private final static String[] INFO_DESC = { "KIND:		         ",
	    "Version:                    ", "#Cores:                     ",
    ...
    private LinkedList<Integer> receiveByte(int n) throws CommunicationException {
	...
	int nROM = 1 << msg.get(5); //Integer.parseInt(msg.substring(10, 12), 16);
	...
	/*if (nROM <0){
	    ...
	    if ((b & 0x20) !=0) nROM+= 32;
	    if ((b & 0x40) !=0) nROM+= 64;
	    if ((b & 0x80) !=0) nROM+= 128;
	}*/
	
	krp = new KrepConfig(nKind, nCores, nIO, nReg, nROM);
     ...
     */
    public int tick(int maxSignals, LinkedList<Signal> inputs, LinkedList<Signal> outputs)
	...
	/*rt=0;
	send(TRACE_COMMAND);
	int bytes = (krp.getIrom()+1)/8;
	String t = receive(2*bytes);
	for( i=0; i<bytes; i++){
	    String s = t.substring(2*i, 2*(i+1));
