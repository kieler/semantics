...
public class KasmAssembler implements IAssembler {
	...
	try {
	    ...
	    result = instructions.toArray(new String[0]);
	} catch (final IOException e) {
	    throw new ParseException(pos+ ": Could not read the listing file: "
		    + e.getMessage());
	}
	...
	while (tokenizer.hasMoreTokens()) {
	    ...
	    if (line.startsWith("[")){
		line = line.substring(line.lastIndexOf('}')+1);	   // if (line.length() >= 2 && !line.startsWith("%")
