...
public class Tools {
    ...
    /**
     * Runs a runnable with progress in the UI Thread of the
	...
	try {
	    ...
	    PlatformUI.getWorkbench().getProgressService().run(true, false, runnable);// .runInUI(context, runnable, null);
	} catch (Exception e) {
   ...
   /**
    * @param i integer containing a byte
 ...
 * current Display thread. Necessary because of SWT's multi-threading principles.
 * 
...
/*class DiagramRunnable implements Runnable {
    ...
    @Override
    public void run() {
	Exception alibiException = new Exception(exc.toString());
	Status s = new Status(IStatus.ERROR, null, exc.getMessage(),
		alibiException);
