...
import de.cau.cs.kieler.krep.evalbench.ui.*;
import de.cau.cs.kieler.krep.evalbench.ui.editors.*;
import de.cau.cs.kieler.krep.evalbench.ui.views.*;
...
public class CommonLayer implements IPartListener {
    ...
    private ICommunicationProtocol currentProtocol;
    ...
    private AssemblerEditor currentEditor;
    ...
    private IAssembler currentProgram;
	...
	/*
	 * IStatus status; if(exception!=null){ status = new
	 * Status(IStatus.ERROR, Activator.PLUGIN_ID, 1, exception.getMessage(),
	 * exception); }else{ status = Status.CANCEL_STATUS;// new
	 * Status(IStatus.ERROR, Activator.PLUGIN_ID, 1, //message, ); }
	 * ErrorDialog.openError(PlatformUI.getWorkbench().getDisplay()
	 * .getActiveShell(), title, message, status);
	 */
    }
     ...
     */
    private static ConnectionView logConnection(String errorTitle,
     ...
     */
    public void loadProgram(final IAssembler program, IProgressMonitor monitor)
     ...
     */
    public void loadProgram(IProgressMonitor monitor)
	    throws CommunicationException, LoadException {
