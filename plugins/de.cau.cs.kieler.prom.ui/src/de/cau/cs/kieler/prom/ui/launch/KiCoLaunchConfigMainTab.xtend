 ...
 * @author aas
 */
class KiCoLaunchConfigMainTab extends AbstractKiCoLaunchConfigTab implements IProjectHolder {
    ...
    private def String checkErrors() {
        ...
        // Project exists
        val proj = PromPlugin.findProject(project.text)
        if (proj == null) {
    ...
    private def void updateEnabled() {
        ...
        val enabled = (PromPlugin.findProject(project.text) != null)
     ...
     */
    override getProject() {
        return PromPlugin.findProject(project.text)
    }
