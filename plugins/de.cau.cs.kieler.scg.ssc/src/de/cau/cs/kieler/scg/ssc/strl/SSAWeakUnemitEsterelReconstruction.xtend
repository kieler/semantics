...
class SSAWeakUnemitEsterelReconstruction extends AbstractProductionTransformation implements Traceable {
    ...
    def Program transform(SCGraph scg, KielerCompilerContext context) {
        ...
        if (hasSSA) {
            ...
            module.body.statements += createLocalSignalDecl => [
                ...
                module.body.statements.immutableCopy.forEach[it.replace(it.translate)]
            }
    ...
    private def dispatch Statement translate(LocalSignalDecl lsig) {
        val sigs = lsig.signalList as LocalSignal
        for (localSignal : sigs.signal.immutableCopy) {
            ...
            if (ssaDelc != null) {
                for (vo : ssaDelc.valuedObjects) {
                    sigs.signal.add(sigs.signal.indexOf(localSignal), createISignal => [
    ...
    private def dispatch Statement translate(Suspend suspend) {
        ...
        val expr = asm.expression.translateExpr
        suspend.delay.event.expr = expr
        if (!(expr instanceof de.cau.cs.kieler.esterel.kexpressions.ValuedObjectReference)) {
            suspend.delay.event.FB = "["
            suspend.delay.event.EB = "]"
        }
        return suspend
    }
