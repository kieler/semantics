...
import static de.cau.cs.kieler.scg.ssc.scl.SSASCL2SSASignalSCL.*
 ...
 */
class SSASignalSCL2SSAEst extends AbstractProductionTransformation {

    // -------------------------------------------------------------------------
    // --                 K I C O      C O N F I G U R A T I O N              --
    // -------------------------------------------------------------------------
    override getId() {
        return "ssasigscl2ssastrl"
    }
    ...
    override getRequiredFeatureIds() {
        return newHashSet(SSASignalSCLFeature.ID)
    }
    ...
    // -- Transformation 
    // -------------------------------------------------------------------------
    // TODO Currently only instantaneous, acyclic, pure signals
    ...
    def Program transform(SCLProgram scl, KielerCompilerContext context) {
        ...
        val interf = createModuleInterface
        module.interface = interf

        for (decl : scl.declarations) {
        ...
        if (interf.intSignalDecls.empty) {
            interf.intSignalDecls += createOutput => [
                signals += createISignal => [
                    name = "ERROR"
                ]
            ]
        }
    ...
    private dispatch def Statement translate(Conditional cond) {
        if (cond.isPresentTest) {
            ...
            if (!cond.elseStatements.empty) {
                present.elsePart = createElsePart => [
    ...
    private dispatch def Expression translateExpression(ValuedObjectReference ref) {
        return if (ref.eContainer instanceof Conditional && (ref.eContainer as Conditional).isPresentTest) {
    ...
    private def isPresentTest(Conditional cond) {
        val inst = cond.eContainer as InstructionStatement
        return inst?.annotations?.exists[name == PRESENT_TEST]
    }
}
