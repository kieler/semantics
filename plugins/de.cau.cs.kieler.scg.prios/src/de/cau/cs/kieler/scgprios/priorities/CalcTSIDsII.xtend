...
class CalcTSIDsII {
    ...
    def HashMap<Node, Integer> calculateTSIDs(List<Node> nodes, HashMap<Node, Integer> nodePriorities, LinkedList<LinkedList<Node>> sccs){
        ...
        numberThreadSegments(rootnode, entrynodes.length, nodePriorities, sccs, nodesOfPath)
        tsIDs
    }
    
    def LinkedList<Node> numberThreadSegments(Node node, int i, HashMap<Node,Integer> nodePriorities, LinkedList<LinkedList<Node>> sccs, LinkedList<Node> nodesOfPath){
        nodesOfPath.add(node)
        if (!(tsIDs.containsKey(node)) || !nodesOfPath.contains(node)){
            if (!(node instanceof Join)){
                ...
                if (node instanceof Exit){
                    ...
                    for (d : decendants){
                        var path = numberThreadSegments(d, i, nodePriorities, sccs, nodesOfPath)
                    ...
                    var path = numberThreadSegments(firstentry, i, nodePriorities, sccs, nodesOfPath)
                    newNodesOfPath.addAll(path)
                    for (e : entrynodes){
                        nextTSID = nextTSID - 1;
                        path = numberThreadSegments(e, (nextTSID+1), nodePriorities, sccs, nodesOfPath)             
                        newNodesOfPath.addAll(path)
                    }
                    tsIDs.put((node as Fork).join,tsIDs.get(((entrynodes.last) as Entry).exit))
                    numberThreadSegments((node as Fork).join.next.target, tsIDs.get(((entrynodes.last) as Entry).exit), nodePriorities, sccs, nodesOfPath)
                    entrynodes.addFirst(firstentry)
                } else{
                    tsIDs.put(node,i)
                    for (c : helper.getAllChildrenOfNode(node)){
                        var path = numberThreadSegments(c, i, nodePriorities, sccs, nodesOfPath)
                        newNodesOfPath.addAll(path)
                    }
