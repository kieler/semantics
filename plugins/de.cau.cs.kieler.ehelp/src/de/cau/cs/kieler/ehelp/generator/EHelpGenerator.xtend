	...
	def void refreshAllWebBrowsers(EObject root) {
		PlatformUI.workbench.display.asyncExec(new Runnable {
			override run() {
				...
				val editorRefs = PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage().editorReferences;
					...
					if (lastWebbrowser !== null) {
						...
						if (EHelpConsts.autoOpenWebbrowser) {
							PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage().openEditor(
								new WebBrowserEditorInput(websiteUri.toURL()), WebBrowserEditor.WEB_BROWSER_EDITOR_ID);
						}
					}
				...
				PlatformUI.workbench.display.asyncExec(new Runnable {
					override run() {
						...
						PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage().bringToTop(xtextEditor);
