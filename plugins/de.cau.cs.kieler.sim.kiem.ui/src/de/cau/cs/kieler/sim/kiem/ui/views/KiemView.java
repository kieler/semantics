...
public class KiemView extends ViewPart implements ISaveablePart2 {
    ...
    private void refreshEnabledDisabledTextColors() {
        ...
        for (int c = 0; c < kIEMInstance.getDataComponentWrapperList().size(); c++) {
            ...
            try {
                // update text colors
                viewer.getTree().getItem(c - disabledCounter).setForeground(
                        currentColor);
                // enable sub items
        ...
        actionEnableDisable = new Action() {
            ...
            public void run() {
                ...
                try {
                    ...
                    KiemPlugin.getDefault().checkForSingleEnabledMaster(false, firstDataCompWrapper);
                    updateView(true);
                } catch (Exception e) {
                ...
                if ((KiemPlugin.getDefault().getCurrentMaster() != null)
                    ...
                    if (kIEMInstance.initExecution()) {
                        ...
                        if (value != null) {
                            try {
                                long step = Long.parseLong(value);
                                kIEMInstance.getExecution().runExecutionPause(
                                        step);
                            } catch (Exception e) {
                                // no error should appear here
                            }
    ...
    public int promptToSaveOnClose() {
        return KiemPlugin.getDefault().promptToSaveOnClose(viewer.getControl()
                    .getShell());
    }
