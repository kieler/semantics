...
int          _tickInstrCnt;          //!< Instructions in one tick
double       _tickCycleCnt;          //!< Clock cycles in one tick
...
#define _case            case
#define _break           break
#define _goto(label)     _state = label; break
#define _initgoto(label) _state = label
...
#define _initgoto(label)   goto label
#define _deref(label)      *label
#define _ref(label)        &&label
...
#define initdispatch()   selectCid(); _initgoto(_deref(_pc[_cid]))
 ...
 * (TRANS) or JOIN.
 *
 * The flag _notInitial can be set to some exotic value ("12345678") to
 * catch (most) cases where an automaton reset has been forgotten.
 */
#define _notInitialDetect 1
#ifdef _SC_SUPPRESS_ERROR_DETECT
#define _SC_ERROR_DETECT_NORESET
#else
#define _SC_ERROR_DETECT_NORESET					\
  if (_notInitial != _notInitialDetect) {					\
...
// ===================================================================
//! Instruction counting/Timing/Tracing
 ...
 */
#ifndef _SC_NOTRACE
  #define instrCntIncr _tickInstrCnt++;
  #define instrCntIncrc _tickInstrCnt++,
  #define instrCntDecr _tickInstrCnt--;
#else
  #define instrCntIncr
...
// ===================================================================
//! Time measurements
//#ifndef _SC_NOTRACE             // Tracing off -> Timing off
//#define _SC_TIMER
//#endif
...
#define _starttimerN gettimeofday(&_tp1, NULL);	\
  _tSum = _tMax = _tMin = 0;			\
