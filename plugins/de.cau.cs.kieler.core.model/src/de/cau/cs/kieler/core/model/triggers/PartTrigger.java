...
public class PartTrigger extends AbstractTrigger implements IPartListener /*
	...
	/**
	 * The default property containing the editorInput path if the active is an
	 * editor.
	 */
	...
	// private void partChanged(final IWorkbenchPart part) {
	// if (this.isActive() && this.lastActive != part) {
	// this.lastActive = part;
	// PartState state = new PartState(part);
	//
	...
	// .getLocationURI() != null) {
	// IPath editorInputPath = ((FileEditorInput)
	// editor.getEditorInput()).getPath();
	...
	public void partActivated(final IWorkbenchPart part) {
		...
		if (this.isActive()) {
			PartState state = new PartState(this.currentActivePart,
			...
			if (isEditorReference) {
				...
				if (editor.getEditorInput().getClass()
						.equals(FileEditorInput.class)
						&& ((FileEditorInput) editor.getEditorInput())
								.getFile() != null
						&& ((FileEditorInput) editor.getEditorInput())
								.getFile().getLocationURI() != null) {
					IPath editorInputPath = ((FileEditorInput) editor
	...
	public void partDeactivated(final IWorkbenchPart part) {
		if (this.currentActivePart != null
				&& this.currentActivePart.equals(part)) {
			this.currentActivePart = null;
			if (this.isActive()) {
				EventType type = EventType.VIEW_DEACTIVATED;
				if (part instanceof IEditorPart) {
					type = EventType.ACTIVE_EDITOR_DEACTIVATED;
					this.trigger(new EditorState(this.currentActivePart,
							this.currentActiveEditor, type));
				}
				this.trigger(new PartState(this.currentActivePart,
						this.currentActiveEditor, type));
			}
		}
	}
	...
	public void partClosed(final IWorkbenchPart part) {
		...
		if (part instanceof IEditorPart) {
			if (this.currentActiveEditor != null
					&& this.currentActiveEditor.equals(part)) {
		...
		if (this.isActive()) {
			PartState state = new PartState(part, this.currentActiveEditor,
					type);

			if (part instanceof IEditorPart) {
				IEditorPart editor = (IEditorPart) part;
				PartState state2 = new EditorState(part,
						this.currentActiveEditor, type);
	...
	public void partActivated(final IWorkbenchPartReference partRef) {
		...
		if (partRef instanceof IEditorReference) {
			this.currentActiveEditor = ((IEditorReference) partRef)
					.getEditor(false);
			type = EventType.EDITOR_ACTIVATED;
			isEditorReference = true;
		}
	...
	public void partDeactivated(final IWorkbenchPartReference partRef) {
		if (this.currentActivePart != null
				&& this.currentActivePart.equals(partRef.getPart(false))) {
			this.currentActivePart = null;
			if (this.isActive()) {
				EventType type = EventType.VIEW_DEACTIVATED;
				if (partRef instanceof IEditorReference) {
	...
	public static enum EventType {
		...
		/**
		 * The active editor has been hidden and is kept on. This might occur if
		 * a view is maximized, which often leads to strange behavior if
		 * PlatformUI....getActiveEditor is used.
		 */
		ACTIVE_EDITOR_HIDDEN,

		/**
		 * The active editor is visible again and is kept on. This might occur
		 * if a view maximized view is re-normalized, which led to
		 * PlatformUI....getActiveEditor returns null in the past.
		 */
	...
	/**
	 * Trigger states containing information of the current active work bench
	 * part.
	 * 
	 * @author chsch
	 */
	public static class PartState extends AbstractTriggerState implements
			IPropertyHolder {
		...
		public PartState(final IWorkbenchPart theCurrentActivePart,
				final IEditorPart theCurrentActiveEditorPart,
				final EventType theEventType) {
		 ...
		 *         If a close event is indicated this method returns the part
		 *         being closed.
		 */
		...
		/**
		 * Convenience tester, whether current active editor is current actual
		 * active workbench part.
		 * 
		 * @return true if the known editor is the actual active part, false
		 *         otherwise.
		 */
	...
	/**
	 * A specialized {@link PartState} allowing to restrict the triggering of
	 * combinations sensible to workbench part events.
	 * 
	 * @author chsch
	 */
	public static class EditorState extends PartState {
		...
		public EditorState(final IWorkbenchPart theCurrentActivePart,
				final IEditorPart theCurrentActiveEditorPart,
				final EventType theEventType) {
			super(theCurrentActivePart, theCurrentActiveEditorPart,
					theEventType);
		}
	}
