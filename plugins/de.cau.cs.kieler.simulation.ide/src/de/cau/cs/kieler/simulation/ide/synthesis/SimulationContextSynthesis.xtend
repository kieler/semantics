...
class SimulationContextSynthesis extends AbstractDiagramSynthesis<SimulationContext> {
    ...
    override KNode transform(SimulationContext model) {
        ...
        val rootNode = (new MessageModel("Simulation", message, KiCoolIdeModule.PLUGIN_ID, "icons/play-button.png", 150)).transform
        // Add action
        if (rootNode !== null && !rootNode.children.empty) {
            ...
            if (icon !== null) {
                icon.addSingleClickAction(AbstractStartSimulationAction.ID)
                icon.addDoubleClickAction(AbstractStartSimulationAction.ID)
            }
            val button = rootNode.eAllContents.filter(KText).filter[message.equals(text)].head
            button.foreground = Colors.BLUE
            button.addSingleClickAction(AbstractAddCoSimulationAction.ID)
            button.addDoubleClickAction(AbstractAddCoSimulationAction.ID)
        }
        return rootNode;
    }
