    ...
    override initialize() throws KiemInitializationException {
        ...
        val Runnable run = [|
            for (context : contextsCirc) {
                ...
                // -------------------------------------------------------------
                // Get Tracing information from SSASCG -> Circuit             --
                // -------------------------------------------------------------
                var Tracing tracing
                var Multimap<Object, Object> traceMap
                val compRes = context.getProperty(KiCoProperties.COMPILATION_RESULT)
                if (compRes != null) {
                    val ssascg = compRes.transformationIntermediateResults.findFirst [
                        id == CircuitFeatures::SCG2SSASCG_ID
                    ].result as SCGraph
                    tracing = compRes.getAuxiliaryData(Tracing).head
                    if (tracing != null) {
                        traceMap = tracing.getMapping(circuit, ssascg)
                ...
                for (node : circuit.eAllContents.filter(Actor).toList) {
                    ...
                    if (atomicActor && (node.name != null) && (node.type != null)) {
                           System.out.println(node.name + " tracing: " + traceMap.get(node))
