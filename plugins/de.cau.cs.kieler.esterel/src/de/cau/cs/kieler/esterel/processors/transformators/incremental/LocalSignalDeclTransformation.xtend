...
class LocalSignalDeclTransformation extends InplaceProcessor<EsterelProgram> {
    ...
    override process() {
        if (true) {
            if (environment.getProperty(SCEstIntermediateProcessor.NEXT_STATEMENT_TO_TRANSFORM) instanceof LocalSignalDeclaration) {
                val LocalSignalDeclaration localS = environment.getProperty(SCEstIntermediateProcessor.NEXT_STATEMENT_TO_TRANSFORM) as LocalSignalDeclaration
                transform(localS)
            }
            else {
