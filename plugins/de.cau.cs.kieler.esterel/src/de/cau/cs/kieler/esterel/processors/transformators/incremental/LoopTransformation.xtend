...
class LoopTransformation extends InplaceProcessor<EsterelProgram> {
    ...
    override process() {
        if (true) {
            if (environment.getProperty(SCEstIntermediateProcessor.NEXT_STATEMENT_TO_TRANSFORM) instanceof Loop) {
                val Loop loop = environment.getProperty(SCEstIntermediateProcessor.NEXT_STATEMENT_TO_TRANSFORM) as Loop
                transform(loop)
            }
            else {
