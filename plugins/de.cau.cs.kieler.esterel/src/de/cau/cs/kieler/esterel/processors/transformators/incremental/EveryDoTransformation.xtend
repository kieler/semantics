...
class EveryDoTransformation extends InplaceProcessor<EsterelProgram> {
    ...
    override process() {
        if (true) {
            if (environment.getProperty(SCEstIntermediateProcessor.NEXT_STATEMENT_TO_TRANSFORM) instanceof EveryDo) {
                val EveryDo ed = environment.getProperty(SCEstIntermediateProcessor.NEXT_STATEMENT_TO_TRANSFORM) as EveryDo
                transform(ed)
            }
            else {
