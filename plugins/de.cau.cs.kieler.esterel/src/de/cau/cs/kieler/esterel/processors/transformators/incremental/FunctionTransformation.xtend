...
class FunctionTransformation extends InplaceProcessor<EsterelProgram> {
    ...
    override process() {
        if (true) {
            if (environment.getProperty(SCEstIntermediateProcessor.NEXT_STATEMENT_TO_TRANSFORM) instanceof EsterelFunctionCall) {
                val EsterelFunctionCall esterelFunctionCall = environment.getProperty(SCEstIntermediateProcessor.NEXT_STATEMENT_TO_TRANSFORM) as EsterelFunctionCall
                transform(esterelFunctionCall)
            }
            else {
