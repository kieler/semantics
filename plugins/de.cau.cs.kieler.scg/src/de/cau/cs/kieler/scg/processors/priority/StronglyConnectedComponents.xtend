...
class StronglyConnectedComponents {
    ...
    static val SCHIZO_ANNOTATION = "Schizophrenic"
    static val DEPTH_JOIN_ANNOTATION = "isDepthJoin"
     ...
     */
    def LinkedList<LinkedList<Node>> findSCCs(Iterable<Node> nodes) {
        ...
        for(scc : sccList) {
            ...
            if (scc.head.hasAnnotation(SCHIZO_ANNOTATION) && scc.length > 1) {
                ...
                if(findSchizoDependencyCycle(scc)) {
                    ...
                    scc.head.removeAnnotations(SCHIZO_ANNOTATION)
                    // Recursively call the scc-analysis with all nodes except the depth-join annotated earlier
                    var subSCCCalc = (injector.getInstance(StronglyConnectedComponents))
