...
class CopyPropagation extends AbstractProductionTransformation {
    ...
    def SCGraph transform(SCGraph scg) {
        ...
        val condAssignments = assignments.filter[
            it.expression.class.typeName.equals(OperatorExpressionImpl.typeName)
                && (it.expression as OperatorExpressionImpl).operator.getName().equals("LOGICAL_OR")
                && (it.expression as OperatorExpressionImpl).subExpressions.length == 1
        ]
        // extract pre assignemts like X = PRE_Y
        val preAssignments = assignments.filter[
            it.expression.class.typeName.equals(OperatorExpressionImpl.typeName)
                && (it.expression as OperatorExpressionImpl).operator.getName().equals("PRE")
        ]
        // extract not assignemnts like  X = !(Y)
        val notAssignments = assignments.filter[
            it.expression.class.typeName.equals(OperatorExpressionImpl.typeName)
                && (it.expression as OperatorExpressionImpl).operator.getName().equals("NOT")
        ]
        /* END FILTERING */
