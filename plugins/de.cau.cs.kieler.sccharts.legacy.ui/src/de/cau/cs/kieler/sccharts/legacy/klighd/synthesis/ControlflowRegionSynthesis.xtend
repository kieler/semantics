...
import de.cau.cs.kieler.sccharts.legacy.klighd.actions.LegacyReferenceExpandAction
import de.cau.cs.kieler.sccharts.legacy.klighd.synthesis.hooks.actions.LegacyMemorizingExpandCollapseAction
    ...
    override performTranformation(ControlflowRegion region) {
        ...
        if (!region.states.empty) {
            ...
            node.addRegionFigure => [
                ...
                addButton("[-]" + label).addDoubleClickAction(LegacyMemorizingExpandCollapseAction.ID);
            ]
...
//        node.setLayoutOption(CoreOptions::SPACING_NODE, 40);
        ...
        node.addRegionFigure => [
            ...
            addButton("[-]").addDoubleClickAction(LegacyReferenceExpandAction::ID);
        ]
