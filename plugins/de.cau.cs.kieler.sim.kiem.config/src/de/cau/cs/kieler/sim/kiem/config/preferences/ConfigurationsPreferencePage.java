...
public class ConfigurationsPreferencePage extends PreferencePage implements
    ...
    private void buildPage(final Composite parent) {
        ...
        KiemProperty[] properties = configuration.getInternalDefaultProperties();
        ...
        alwaysUseDefaultsButton.setToolTipText(Tools.IGNORED_KEYS_BUTTON_TOOLTIP);
        ...
        propertiesGroup.setLayoutData(new GridData(SWT.BEGINNING, SWT.BEGINNING, true, true));
    ...
    private boolean saveProperties() {
        for (KiemIntegerPropertyInputField input : inputs) {
            if (!input.save()) {
                Tools.showErrorDialog("Invalid input on text field.", page.getShell());
                input.update();
                return false;
            }
        }
     ...
     * 
     * @param comboId
     *            either ContributionManager.RECENT_COMBO or ContributionManager.MATCHING_COMBO
     * @param enabled
    ...
    /**
     * Listener for handling the different events that happen on the preference page.
     * 
     * @author soh
     * @kieler.rating 2010-01-27 proposed yellow
     */
        ...
        public final void handleEvent(final Event event) {
            try {
                ...
                if (event.widget == matchingCheck) {
                    toggleCombo(ContributionManager.MATCHING_COMBO, matchingCheck.getSelection());
                }
                if (event.widget == recentCheck) {
                    toggleCombo(ContributionManager.RECENT_COMBO, recentCheck.getSelection());
                }
    ...
    /**
     * Handles a Text input field with label and tooltip for one of the default KIEM properties.
     * <p>
     * The class also provides a save method to write the contents of the text field to the back
     * end.
     * 
     * @author soh
     * @kieler.rating 2010-01-27 proposed yellow
