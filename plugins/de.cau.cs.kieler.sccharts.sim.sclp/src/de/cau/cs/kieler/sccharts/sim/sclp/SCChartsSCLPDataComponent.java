 ...
 */
package de.cau.cs.kieler.sccharts.sim.sclp;
...
import de.cau.cs.kieler.sclp.SCLPExecution;
import de.cau.cs.kieler.sccharts.State;
import de.cau.cs.kieler.sccharts.sim.sclp.xtend.SCLPSimulationSCChart;
import de.cau.cs.kieler.sccharts.sim.sclp.xtend.SCLPSimulationSCG;
 ...
 */
public class SCChartsSCLPDataComponent extends JSONObjectSimulationDataComponent implements
    ...
    private static final String KIEM_PROPERTY_NAME_CCOMPILER = "SCLP-Compiler";
    ...
    /** The Constant KIEM_PROPERTY_DEFAULT_COMPILETRANSFORMATIONS. */
    private static final String KIEM_PROPERTY_DEFAULT_LOWLEVELTRANSFORMATIONS = "SCLP";

    /** The C execution object for concurrent execution. */
    private SCLPExecution cExecution = null;
    ...
    public SCChartsSCLPDataComponent() {
    ...
    public String getDataComponentId() {
        return "de.cau.cs.kieler.sccharts.sim.sclp";
    }
     ...
     */
    public SCLPExecution getCExecution() {
        return cExecution;
    }
    ...
    private String getBundlePath(String subDirectory) {
        Bundle bundle = Platform.getBundle(SCChartsSimSCLPPlugin.PLUGIN_ID);
            ...
            if (myModel != null && kExpressionExtension.getValuedObjects(myModel) != null) {
                for (ValuedObject valuedObject : kExpressionExtension.getValuedObjects(myModel)) {
                    ...
                    if (kExpressionExtension.isOutput(valuedObject)) {
                        ...
                        if (signalName.startsWith(SCChartsSimSCLPPlugin.AUXILIARY_VARIABLE_TAG_STATE)) {
                            outputStateList.add(signalName);
                        } else if (signalName
                                .startsWith(SCChartsSimSCLPPlugin.AUXILIARY_VARIABLE_TAG_TRANSITION)) {
...
//            }
            ...
            if (stateOrSCG instanceof State) {
                ...
                SCLPSimulationSCChart cSimulationSCChart = Guice.createInjector().getInstance(SCLPSimulationSCChart.class);
                System.out.println("16");
                cSimulation = cSimulationSCChart.transform((State)stateOrSCG, "10000").toString();
            }
            else if (stateOrSCG instanceof SCGraph) {
                System.out.println("15");
                SCLPSimulationSCG cSimulationSCG = Guice.createInjector().getInstance(SCLPSimulationSCG.class);
                System.out.println("16");
            ...
            // Compile
            cExecution = new SCLPExecution(outputFolder, false);
                ...
                if (outputArray != null) {
                    ...
                    for (int i = 0; i < outputArray.length(); i++) {
                        ...
                        if (output.get(outputName) instanceof JSONObject) {
                            ...
                            if (outputName.startsWith(SCChartsSimSCLPPlugin.AUXILIARY_VARIABLE_TAG_STATE)) {
                                if (present) {
                                    if (activeStatesBuf.length() > 0) {
                                        activeStatesBuf.append(",");
                                    }
                                    String activeStateName = outputName.substring(SCChartsSimSCLPPlugin.AUXILIARY_VARIABLE_TAG_STATE.length());
                                    activeStatesBuf.append(activeStateName);
                                }
                            } else if (outputName.startsWith(SCChartsSimSCLPPlugin.AUXILIARY_VARIABLE_TAG_TRANSITION)) {
                                if (present) {
                                    if (activeTransitionsBuf.length() > 0) {
                                        activeTransitionsBuf.append(",");
                                    }
                                    String activeTransitionName = outputName.substring(SCChartsSimSCLPPlugin.AUXILIARY_VARIABLE_TAG_TRANSITION.length());
