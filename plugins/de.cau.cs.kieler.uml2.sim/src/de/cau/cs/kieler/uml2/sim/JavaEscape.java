...
public class JavaEscape {
	...
	public static String getId(org.eclipse.uml2.uml.Vertex vertex) {
		...
		if (vertex.getName() != null) {
			vertexId = vertex.getName();	
		}
		vertexId += vertex.eResource().getURIFragment(vertex).toString();
		return vertexId;
	}
	...
	public static String getId(org.eclipse.uml2.uml.Transition transition) {
		...
		if ((transition.getSource().getName() != null)
			...
			transitionId = transition.getSource().getName() + "2"
					+ transition.getTarget().getName();
		}
		transitionId += transition.eResource().getURIFragment(transition)
				.toString();
		return transitionId;
	}
	...
	// else return TransitionLabel.substring(TransitionLabel.indexOf("@")+1,
	// TransitionLabel.indexOf("(name")-1) ;
	// }
