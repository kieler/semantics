 ...
 */
@Singleton
class ScchartsStructuredProgrammingServerExtension implements ILanguageServerExtension{
    //only injection that works as intendet 
    // used to change transitions
    @Inject extension SCChartsTransitionExtensions

    //all injections are problematic since they return null! 
    //used to communicate with the frontend eg sending error messages
    //@inject
    static KGraphLanguageClient client;
    
    // used to get the client uri
    //@inject
    static KGraphDiagramState diagramState

    // used to get the model from a given uri
    //@inject
    static KGraphLanguageServerExtension languageServer
    ...
    def setKGraphLanguageServerExtension(KGraphLanguageServerExtension serverExt) {
        this.languageServer = serverExt
    }
