...
class DebugBreakpointManager {
    ...
    // Registers if the model file has been changed since it was loaded
    val isModelDirty = <String, Boolean> newHashMap
    ...
    val pendingBreakpoints = <IJavaBreakpoint> newLinkedList
    
    // Java files that may have auto-set breakpoints in them
    val javaFilePaths = <IPath, ModelBreakpointManager> newHashMap
    ...
    private new(){
        ...
        // Create resource change listener
        val rcl = new IResourceChangeListener() {
           override resourceChanged(IResourceChangeEvent e) {
               if (e.type == IResourceChangeEvent.POST_CHANGE) {
                   DebugBreakpointManager.instance.processResourceChangeEvent(e)
               }
           } 
        }
        ResourcesPlugin.workspace.addResourceChangeListener(rcl)
    ...
    def setModel(String text) {
        ...
        if (currentModel === null || diagramView?.needsInit || (text != currentModelPath)
            ...
            || isModelDirty.get(currentModelPath)) {
            
            // Only re-parse / reload the model if it is not the same one as before
            if (text != currentModelPath || isModelDirty.get(currentModelPath)) {
                currentModelPath = text
                val mapEntry = pathToModel.get(text)
                if (mapEntry !== null && !isModelDirty.get(text)) {
                    ...
                    if (mapEntry === null) { 
                        ...
                        // reload the model into the ModelBreakpointManager and reapply all highlightings
                        val bpmanager = pathToBpManager.get(text)
                        bpmanager.reloadModel(currentModel)
                        val hl = pathToHighlighter.get(text)
                        hl.reloadModel(currentModel, bpmanager)
                        // TODO ensure that highlighting and BP manager can cope with a new model
                    }
                }
            }
            isModelDirty.put(currentModelPath, false)
    ...
    def void addJavaFilePath(IPath path, ModelBreakpointManager bpManager) {
        javaFilePaths.put(path, bpManager)
    }
    ...
    def void processResourceChangeEvent(IResourceChangeEvent e) {
        var currentModelChanged = false
        // Detect changes in source model files (.sctx)
        for (path : pathToModel.keySet) {
            val delta = e.delta.findMember(new Path(path))
            if (delta !== null) {
                isModelDirty.put(path, true)
        ...
        // Detect changes in generated code files to move breakpoints
        val changedJavaFiles = <IPath> newLinkedList
        for (path : javaFilePaths.keySet) {
            val delta = e.delta.findMember(path)
            if (delta !== null && ((delta.flags.bitwiseAnd(IResourceDelta.CONTENT)) != 0)) {
                changedJavaFiles.add(path)
            }
        }
        
        if (currentModelChanged && modelBeingDisplayed) {
            // Delay execution by using a job to avoid running into a locked resource tree
            // (Since this code is executed in the ResourceChangeEvent handler)
            Job.create("Reload Model", new IWorkspaceRunnable() {
                override run(IProgressMonitor m) {
                    model = currentModelPath
                }
            }).schedule
        }
        
        if (!changedJavaFiles.empty) {
            Job.create("Reload Breakpoints", new IWorkspaceRunnable() {
                override run(IProgressMonitor m) {
                    for (path : changedJavaFiles) {
                        javaFilePaths.get(path).resetBreakpoints(path)
                    }
                }
            }).schedule
        }
