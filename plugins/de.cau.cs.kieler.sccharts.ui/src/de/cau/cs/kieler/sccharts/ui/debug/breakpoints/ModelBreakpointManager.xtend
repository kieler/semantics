...
class ModelBreakpointManager {
    ...
    val javaFilePaths = <String> newHashSet
    ...
    def reloadModel(SCCharts model) {
        // State Breakpoints
    ...
    /**
     * After a source file with breakpoints has been changed, all breakpoints must be removed and added again
     * to ensure that they still are in the correct lines.
     */
    def resetBreakpoints (IPath path) {
        for (state : new HashSet(statesWithBreakpoint)) {
            state.toggleBreakpoint
            state.addBreakpointWithPath(path)
        }
        for (transition : new HashSet(transitionsWithBreakpoint)) {
            transition.toggleBreakpoint
            transition.addBreakpointWithPath(path, false)
        }
        for (transition : new HashSet(transitionsWithCheckBreakpoint)) {
            transition.toggleCheckBreakpoint
            transition.addBreakpointWithPath(path, true)
        }
    }
    ...
    def IJavaBreakpoint createBreakpointOnLine(int line, BreakpointType breakpointType, State state, Transition transition) {
        ...
        DebugBreakpointManager.instance.addJavaFilePath(resource.fullPath, this)
    ...
    private def addBreakpointWithPath(State state, IPath path) {
        val file = ResourcesPlugin.workspace.root.getFile(path)
        val nameHash = DebugAnnotations.getFullNameHash(state)
        val pattern = "\\s\\* State [a-zA-Z0-9]+ \\(" + nameHash + "\\)"
        val transitionPatterns = <String, Transition> newHashMap
        val stateLines = <Integer> newLinkedList
        for (transition : state.outgoingTransitions) {
            transitionPatterns.put(".*// Transition [a-zA-Z]+ \\(Priority \\d+\\) -> [a-zA-Z]+ \\(" 
                + DebugAnnotations.getFullNameHash(transition) + "\\)",
                transition
                )
        }
        
        val fileContents = new LineNumberReader(new InputStreamReader(file.getContents(false)))
        var String currentLine = fileContents.readLine
        val transitionLines = <Integer, Transition> newHashMap
        
        do {
            if (currentLine.matches(pattern)) {
                stateLines.add(fileContents.lineNumber + 2)
            }
            for (transitionPattern : transitionPatterns.keySet) {
                if (currentLine.matches(transitionPattern)) {
                    transitionLines.put(fileContents.lineNumber, transitionPatterns.get(transitionPattern))
                }
            }
        ...
        val checkBreakpoints = <IJavaBreakpoint> newLinkedList
        for (line : transitionLines.keySet) {
            val bp = createBreakpointOnLine(line, BreakpointType.TRANSITION_WATCH_BREAKPOINT, null, transitionLines.get(line))
            checkBreakpoints.add(bp)
        }
        stateToWatchBreakpoints.put(state, checkBreakpoints)
        
    }
    
    def addBreakpointWithPath(Transition transition, IPath path, boolean checkBreakpoint) {
        val file = ResourcesPlugin.workspace.root.getFile(path)
        val nameHash = DebugAnnotations.getFullNameHash(transition)
        val pattern = "// Transition [a-zA-Z]+ \\(Priority \\d+\\) -> [a-zA-Z]+ \\(" + nameHash + "\\)"
        val transitionLines = <Integer> newLinkedList
        ...
        do {
            if (currentLine.matches(pattern)) {
                transitionLines.add(fileContents.lineNumber + 1)
            }
            currentLine = fileContents.readLine
        } while (currentLine !== null)
        ...
        val transitionBreakpoints = <IJavaBreakpoint> newLinkedList
        
        for (line : transitionLines) {
            var IJavaBreakpoint bp
            if (checkBreakpoint) {
                bp = createBreakpointOnLine(line, BreakpointType.TRANSITION_TAKEN_BREAKPOINT, null, transition)
            } else {
                bp = createBreakpointOnLine(line, BreakpointType.TRANSITION_CHECK_BREAKPOINT, null, transition)
            }
            transitionBreakpoints.add(bp)
        }
        
        if (checkBreakpoint) {
            transitionsWithBreakpoint.add(transition)
            highlighter.addBreakpointDecorator(transition)
            transitionToBreakpoint.put(transition, transitionBreakpoints)
        } else {
            transitionsWithCheckBreakpoint.add(transition)
            highlighter.addCheckBreakpointDecorator(transition)
            transitionToCheckBreakpoint.put(transition, transitionBreakpoints)
        }
    }
    
    def Transition getMatchingTransition(SCCharts model, Transition transition) {
        val hash = DebugAnnotations.getFullNameHash(transition)
        val matchingTransitions = <Transition> newLinkedList
        for (rootState : model.rootStates) {
            matchingTransitions.addAll(rootState.allContainedTransitions
                .filter[DebugAnnotations.getFullNameHash(transition) == hash].toList
            )
        }
        return matchingTransitions.head
    }
    ...
    def State getMatchingState(SCCharts model, State state) {
        ...
        for (rootState : model.rootStates) {
            matchingStates.addAll(rootState.allStates.filter[DebugAnnotations.getFullNameHash(it) == hash].toList)
        }
        return matchingStates.head
    }
