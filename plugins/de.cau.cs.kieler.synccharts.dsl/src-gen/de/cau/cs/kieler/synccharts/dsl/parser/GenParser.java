...
public abstract class GenParser extends AbstractXtextParser implements IXtextParser {
	...
	private scdslLexer lexer;
	private scdslParser parser;

	public GenParser(InputStream in) {
		this.reader = new InputStreamReader(in);
	}

	public scdslLexer getLexer() {
		if (lexer == null) {
			try {
				this.lexer = new scdslLexer(new ANTLRReaderStream(reader));
			} catch (IOException e) {
				handleError(e);
			}
		}
	...
	public scdslParser getParser() {
		if (parser == null) {
			parser = new scdslParser(new CommonTokenStream(getLexer()));
		}
		return parser;
	}
	...
	protected void internalLink() {
		...
		try {
			...
			XtendFacade facade = XtendFacade.create(ctx, "de::cau::cs::kieler::synccharts::dsl::Linking");
	...
	public Issues doCheck() {
		...
		try {
			...
			String[] checkFiles = new String[]{"de::cau::cs::kieler::synccharts::dsl::GenChecks","de::cau::cs::kieler::synccharts::dsl::Checks"};
