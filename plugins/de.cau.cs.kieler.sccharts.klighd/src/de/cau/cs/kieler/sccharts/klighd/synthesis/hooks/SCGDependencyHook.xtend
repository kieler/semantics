...
class SCGDependencyHook extends SynthesisActionHook {
    ...
    public static final SynthesisOption SHOW_SCG_DEPENDENCIES = SynthesisOption.createCheckOption(
        "Show SCG Dependencies", false).setCategory(GeneralSynthesisOptions::DEBUGGING).setUpdateStrategy(
    ...
    public static final SynthesisOption SCG_DEPENDENCY_TYPES = SynthesisOption.createChoiceOption("Dependency Types",
        newArrayList(DepType.Elements, DepType.Regions), DepType.Elements).setCategory(
        GeneralSynthesisOptions::DEBUGGING).setUpdateStrategy(SimpleUpdateStrategy.ID)
    // Deactivated to force Simple Update Strategy
    // .setUpdateAction(SCGDependencyHook.ID); // Add this action as updater
    ...
    public static final SynthesisOption SHOW_SELECTED_DEPENDENCIES = SynthesisOption.createCheckOption(
        "Show only Dependencies of selected Elements", false).setCategory(GeneralSynthesisOptions::DEBUGGING).
    ...
    private static final ISelectionChangedListener selectionListener = new ISelectionChangedListener() {
        ...
        override void selectionChanged(SelectionChangedEvent event) {
            ...
            if (viewContext.getOptionValue(SHOW_SCG_DEPENDENCIES) as Boolean &&
                viewContext.getOptionValue(SHOW_SELECTED_DEPENDENCIES) as Boolean) {
                            ...
                            for (EObject source : sources) {
                                for (EObject target : targets) {
                                    ...
                                    if (sourceRegion !== null) {
                                        newsources.clear
                                        newsources.addAll((sourceRegion as KNode).data.filter(KRectangle).map [
                                            it.children.filter(KText).head
                                        ])
                                    }
                                    ...
                                    if (targetRegion !== null) {
                                        newtargets.clear
                                        newtargets.addAll((targetRegion as KNode).data.filter(KRectangle).map [
                                            it.children.filter(KText).head
                                        ])
                                    }
                                    for (EObject newsource : newsources) {
                                        for (EObject newtarget : newtargets) {
                                            if (newsource != newtarget) {
                                                regionEdges.createDependencyEdge(attachNode, newsource, newtarget, dep,
                                                    true)
                                            }
                                        }
                                    }
