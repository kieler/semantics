...
// Rule ScadeProgram
ruleScadeProgram returns [EObject current=null]
@init {
	enterRule();
	HiddenTokens myHiddenTokenState = ((XtextTokenStream)input).setHiddenTokens("RULE_WS", "RULE_SL_COMMENT", "RULE_ML_COMMENT");
}
...
entryRuleScadeEquation returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getScadeEquationRule()); }
	iv_ruleScadeEquation=ruleScadeEquation
	{ $current=$iv_ruleScadeEquation.current; }
	EOF;

// Rule ScadeEquation
ruleScadeEquation returns [EObject current=null]
@init {
	enterRule();
}
@after {
