...
public class UpdateResourceFactoryImpl extends XMIResourceFactoryImpl {
				...
				try {
					dialog.run(true, false, new IRunnableWithProgress() {
						...
						public void run(IProgressMonitor monitor) throws InvocationTargetException,
							...
							monitor.beginTask("Converting Synccharts 0.1 to 0.2!", IProgressMonitor.UNKNOWN);
							doPreprocessing(uri);
							monitor.done();
						}
					});
	...
	/**
	 * Perform pre-processing steps that have to be done before the parser
	 * can start to load the file. This is necessary because some features
	 * that have changed are not recorded by the record unknown features
	 * flag and can cause the file not to be opened.
	 * 
	...
	private void doPreprocessing(final URI uri) {
		...
		try {
			...
			while (s != null) {
					...
					if (s.contains("xmlns:synccharts=\"http://kieler.cs.cau.de/synccharts/0.2\"")) {
						is.close();
						return;
					}
				...
				newString = newString.replaceAll("type=\"UNSIGNED\"", "type=\"unsigned\"");
				...
				newString = newString.replaceAll("xsi:type=\"synccharts:HostCode\"", "xsi:type=\"synccharts:TextualCode\"");
				
				// complex expression with an operator are now called operator expressions
				newString = newString.replaceAll("xsi:type=\"synccharts:ComplexExpression\" operator=\"", "xsi:type=\"synccharts:OperatorExpression\" operator=\""); 
				
				// renaming -> substitution, formal = oldID, actual = newID
				newString = newString.replaceAll("xsi:type=\"synccharts:Renaming\" oldID=\"", "xsi:type=\"synccharts:Substitution\" formal=\"");
				if (newString.contains("xsi:type=\"synccharts:Substitution\"")) {
					newString = newString.replaceAll("newID=\"", "actual=\"");
				}
				
				newString = newString.replaceAll("targetState=\"//@", "DUMMY_targetState=\"//@");
			...
			if (modified) {
				...
				line2 = line2.replace("xmlns:synccharts=\"http://kieler.cs.cau.de/synccharts\"", "xmlns:synccharts=\"http://kieler.cs.cau.de/synccharts/0.2\"");
				lines.set(1, line2);
	...
	private class SyncchartsResourceHandler extends BasicResourceHandler {
		...
		public void postLoad(final XMLResource resource, final InputStream inputStream, final Map<?, ?> options) {
			...
			for(Iterator<Map.Entry<EObject, AnyType>> itr = extMap.entrySet().iterator(); itr.hasNext();) {
				Map.Entry<EObject, AnyType> entry = (Map.Entry<EObject, AnyType>) itr.next();
		 ...
		 * 
		 * @param eObj the resource containing the unknown data.
		 ...
		 * 
		 * @param owner the containing resource
