...
public class ActionsDiagnosticConverterImpl extends DiagnosticConverterImpl {
    ...
    public void convertValidatorDiagnostic(org.eclipse.emf.common.util.Diagnostic diagnostic,
        ...
        if (SCChartsPackage.eINSTANCE.getTransition().isInstance(getCauser(diagnostic))) {
            EStructuralFeature feature = getFeature(diagnostic);
            if (SCChartsPackage.eINSTANCE.getTransition_SourceState().equals(feature)
                    || SCChartsPackage.eINSTANCE.getTransition_TargetState().equals(feature)) {
                return;
            }
        }
