 ...
 * 
 */
class KiVisDataView extends ViewPart implements ISimulationListener {

    /**
     * The id of the view, that is set in the plugin.xml
     */
    public static val VIEW_ID = "de.cau.cs.kieler.kivis.ui.kivisDataView"

    /**
     * The last instance of this view
     */
    @Accessors(PUBLIC_GETTER)
    static var KiVisDataView instance;
        ...
        getViewSite().getActionBars().getToolBarManager() => [
            add(new Action("Open in external Browser", IAction.AS_PUSH_BUTTON) {
                override run() {
                    ...
                    browserSupport.externalBrowser?.openURL(new URL("http://localhost:" + SimulationServer.PORT + "/dataview"))
                }
            })
        ]
