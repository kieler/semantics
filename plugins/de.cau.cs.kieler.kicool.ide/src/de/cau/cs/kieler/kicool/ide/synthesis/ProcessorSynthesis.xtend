...
class ProcessorSynthesis {
    ...
    def KNode processorNode() {
        KiCoolSynthesis.getKGTFromBundle(KiCoolIdeModule.UI_BUNDLE_ID, PROCESSOR_KGT)
    }
...
//        println(nodeId)
    ...
    dispatch def List<KNode> transform(ProcessorGroup processorGroup) {
        val groupNode = KiCoolSynthesis.getKGTFromBundle(KiCoolIdeModule.UI_BUNDLE_ID, PROCESSOR_GROUP_KGT)
            ...
            if (it instanceof ProcessorReference) {
                for (le : lastEntries.filter(ProcessorReference)) {
                    ...
                    if (!KiCoolRegistration.checkProcessorCompatibility(sourceId, targetId)) {
                        ...
                        for (edge : edges) {
                            edge.getContainer.addImage(KiCoolIdeModule.UI_BUNDLE_ID, KiCoolIdeModule.ICON_PATH + COMPATIBILITY_ERROR_ICON) => [
