package de.cau.cs.kieler.uml.viewmanagement;
...
public class ActiveStateFocusCombination extends ACombination {
    ...
    public boolean evaluate(TriggerEventObject triggerEvent) {
        ...
        if (rootEditPart != null) {
            ...
            if (triggerEvent.getAffectedObject() instanceof Vertex) {
                Vertex state = (Vertex) triggerEvent.getAffectedObject();
                // get a List of all parent states
                Collection<EObject> statesToExpand = new ArrayList<EObject>();
                if ((state instanceof State) && (((State)state).getRegions().size() > 0)) {
    ...
    private void init() throws KielerModelException {
        Collection<EObject> states = ModelingUtil.getAllByType(UMLPackage.eINSTANCE
                .getVertex(), rootEditPart);
        for (EObject state : states) {
            // remove the root State, because it represents the whole SM and
            // will not be active at
            // all
            if (((State) state).eContainer() == null
                    || !((((State) state).eContainer()) instanceof Region)) {
                continue;
