...
import de.cau.cs.kieler.yakindu.model.stext.serializer.SynctextSemanticSequencer;
...
public abstract class AbstractSCChartsExpSemanticSequencer extends SynctextSemanticSequencer {

	@Inject
	private SCChartsExpGrammarAccess grammarAccess;
	
	public void createSequence(EObject context, EObject semanticObject) {
		if(semanticObject.eClass().getEPackage() == SCChartsExpPackage.eINSTANCE) switch(semanticObject.eClass().getClassifierID()) {
			case SCChartsExpPackage.DUMMY:
				if(context == grammarAccess.getDummyRule()) {
	 ...
	 */
	protected void sequence_Dummy(EObject context, Dummy semanticObject) {
		if(errorAcceptor != null) {
			if(transientValues.isValueTransient(semanticObject, SCChartsExpPackage.Literals.DUMMY__VARIABLE_DEFINITION) == ValueTransient.YES)
				errorAcceptor.accept(diagnosticProvider.createFeatureValueMissing(semanticObject, SCChartsExpPackage.Literals.DUMMY__VARIABLE_DEFINITION));
		}
		INodesForEObjectProvider nodes = createNodeProvider(semanticObject);
		SequenceFeeder feeder = createSequencerFeeder(semanticObject, nodes);
		feeder.accept(grammarAccess.getDummyAccess().getVariableDefinitionVariableDefinitionParserRuleCall_0(), semanticObject.getVariableDefinition());
		feeder.finish();
	}
}
