...
class TTPTransformation extends AbstractProductionTransformation {
    ...
    def public Object transform(SCGraph scg, KielerCompilerContext context) {
        ...
        if (tracing == null) {
            System.out.println("\nNo tracing could be found. Tracing has to be activated.\n")
        // TODO: transformation typical error
        }
//        if (tracing == null) {
...
// Analyse tracing relation into a node to region mapping
        ...
        if (transformationObject instanceof State) {
            ...
            System.out.println(
                "A transformationObject of type State could not be found.\n 
                The compiler context must contain the original SCChart."
            )
        // TODO: correct error handling for transformation
        }
        ...
        for (Object originElement : mapping.keySet()) {
            if (originElement instanceof Node) {
                ...
                for (Object targetObj : targetElements) {
                    ...
                    if (!(targetElement instanceof State &&
                        ((targetElement as State).hasInnerStatesOrControlflowRegions)) || targetElements.size() == 1) {
                            ...
                            while (targetElement != null && !regionFound) {
                                if (targetElement instanceof Region) {
                                    ...
                                    var int candidateDepth = regionDepth.get(newRegionCandidate);
                                    /*
            ...
            val int highestInsertedTPPNumber = insertTPP(scg, nodeRegionMapping, tppRegionMap, scchartDummyRegion);
        ...
        def private int insertTPP(SCGraph scg, HashMap<Node, Region> nodeRegionMapping,
            ...
            val Region firstSourceRegion = getSourceRegion(firstEdge, nodeRegionMapping, scchartDummyRegion);
            ...
            while (edgeListIterator.hasNext()) {
                ...
                val Region sourceRegion = getSourceRegion(currentEdge, nodeRegionMapping, scchartDummyRegion);
                ...
                if ((targetRegion != null) && (sourceRegion != null)) {
                    ...
                    if (!(sourceRegion.equals(targetRegion))) {
                        ...
                        if (!(redirectedEdges.contains(currentEdge))) {
                            ...
                            val Assignment tpp = insertSingleTPP(scg, currentEdge, tppCounter, redirectedEdges);
                            ...
                            if (targetIncomingEdges.size() > 1) {
                                val Iterator<Link> targetIncomingIterator = targetIncomingEdges.iterator();
        ...
        def private LinkedList<ControlFlow> getEdgesInFixedTraversingOrder(SCGraph scg) {
            ...
            val Iterator<Entry> entryIter = Iterators.filter(ModelingUtil.eAllContentsOfType2(scg, Node), Entry);
            var Node entry = null;
         ...
         */
        def private void traverseSequentialGraphEdges(Node currentNode, LinkedList<ControlFlow> edgeList) {
                ...
                if (currentNode instanceof Exit) {
                    ...
                    if (currentNode instanceof Conditional) {
                        ...
                        if (currentNode instanceof Assignment) {
                            val ControlFlow outgoingEdge = (currentNode as Assignment).getNext();
                            edgeList.add(outgoingEdge);
                            // check whether this node ends a then branch, if so, add the else edge to
                            // the
                            // list also
                            val EList<Link> targetIncomingList = outgoingEdge.getTarget().getIncoming();
                            if (targetIncomingList.size() > 1) {
                                val Iterator<Link> incomingEdgeIterator = targetIncomingList.iterator();
         ...
         */
        def private Region getSourceRegion(ControlFlow controlFlow, HashMap<Node, Region> nodeRegionMapping,
            Region scchartDummyRegion) {
         ...
         */
        def private Assignment insertSingleTPP(SCGraph scg, ControlFlow controlFlow, int tppNumber,
            ...
            val TextExpression tppText = KExpressionsFactory.eINSTANCE.createTextExpression();
