...
class DataflowExtractor extends ExogenousProcessor<CodeContainer, SCCharts> {
    ...
    def SCCharts transform(CodeContainer codeContainer) {
        ...
        val int opts = 8;
        val IASTTranslationUnit ast = GCCLanguage.getDefault().getASTTranslationUnit(fileContent, info, emptyIncludes,
            null, opts, log);
...
//        val IFile file = ResourcesPlugin.workspace.root.getFile(new Path(cFileName))
        ...
        try {
            val IFile file = ResourcesPlugin.workspace.root.getFile(
                new Path("C-Tutorial-Examples/084-user_defined_function.c"))
    ...
    def State createUnknownFuncState(String funcName, IASTFunctionCallExpression funcCall, State parentState,
        ...
        for (arg : funcCall.arguments) {
            ...
            if (arg instanceof IASTIdExpression) {
                ...
                val inputVO = parentDeclarations.map[it.valuedObjects].flatten.findFirst [
                    it.name.substring(0, it.name.indexOf(ssaNameSeperator)).equals(fullVariableName)
                ]
