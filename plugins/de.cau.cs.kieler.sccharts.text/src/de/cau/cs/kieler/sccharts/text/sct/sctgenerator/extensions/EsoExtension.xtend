...
class EsoExtension implements ISCTGeneratorExtension {
    ...
    override onSaveModel(State rootState, IProject project) {
        ...
        if(GENERATE_TRACE.property.value == 1) {            
            ...
            rootState.declarations.filter[it.input].generateTraces()
    ...
    def generateInputsForTick(Iterable<Declaration> inputs) {
    ...
    def generateInputsForTrace(Iterable<Declaration> inputs) {
    ...
    def generateTraces(Iterable<Declaration> inputs) {
