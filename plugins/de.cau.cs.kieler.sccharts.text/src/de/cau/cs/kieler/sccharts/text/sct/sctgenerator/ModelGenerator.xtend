...
class ModelGenerator {
    ...
    protected def Transition createTransitionTrigger(Transition transition) {
        ...
        transition.trigger = createTriggerExpression(rootState.declarations.filter[input].head, triggerDepth)
    ...
    protected def createTransitionEffects(Transition transition) {
        ...
        for (var int i = 0; i < effectCount; i++) {
            transition.effects += createAssignmentEffect(rootState.declarations.filter[output].head)
