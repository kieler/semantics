	...
	def transform(SCGraph scg) {
		...
		for (n : nodes) {
			if (n instanceof Assignment)
				System.out.println(n.valuedObject.name)
		}
		...
		for (n : nodes) {
			if (n instanceof Assignment) {
				...
				if (ssaMap.containsKey(name)) {
					System.out.println("found " + name + " in Map. value is " + ssaMap.get(name))
					val newNode = ScgFactory::eINSTANCE.createAssignment

					val vo = createValuedObject(name + "_" + ssaMap.get(name))
					newNode.valuedObject = vo
					System.out.println("created a new Node with name " + newNode.valuedObject.name)
					val copyExpr = n.assignment.copy
					System.out.println("COPIED expression " + copyExpr.toString)
