	...
	def void transformConditionalNodes(Conditional sourceConditional, Node predecessorNodeThen,
		...
		if (thisNode instanceof Conditional) {
			...
			if (isSSArelevant) {
				...
				val newNode = ScgFactory::eINSTANCE.createAssignment.trace(thisNode, thisNode.valuedObject)
				...
				if (!storeVO.isInput && (ssaMap.get(name) == 1)) {
					val expression = KExpressionsFactory::eINSTANCE.createOperatorExpression.trace
