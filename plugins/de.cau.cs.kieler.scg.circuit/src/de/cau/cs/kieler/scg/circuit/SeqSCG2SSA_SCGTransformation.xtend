...
class SeqSCG2SSA_SCGTransformation extends AbstractProductionTransformation implements Traceable{
	...
	// This map stores the name of each SSA variable and its latest version number.
	val ssaMap = new HashMap<String, Integer>
	
	// This map stores the name of valuedObjects and the valuedObject itself. 
	// The value will be replaced by a new valuedObject each time an SSA variable is created for the key.
	val valuedObjectList = new HashMap<String, ValuedObject>
	
	//This map stores the name of output variables and how often the variables are written to.
	val outputOccurenceCounter = new HashMap<String, Integer>
	
	//This map stores all input output variables for usage in link creation for the circuit transformation.
	...
	def transform(SCGraph scg, KielerCompilerContext context) {
		...
		filterVariablesWithMultipleWrites(scg.nodes.filter(Assignment).toList)
		
		// Create the SSA SCG. Start with entry node and follow the control flow.
		controlFlowStep(entry.next.target, scg)
	 ...
	 * 
	 * @param node next node on control-flow of SCG
	 * @param scg  original SCG
	 */
	def private void controlFlowStep(Node node, SCGraph scg) {

		if (!(node instanceof Exit)) {

			if (node instanceof Assignment) {
				transformAssignmentNodes(node, scg)
				controlFlowStep(node.next.target, scg)
			} else if (node instanceof Conditional) {
				val target = node.^else.target
				transformConditionalNodes(node, node, node, node.^else.target, scg)
				controlFlowStep(target, scg)
			}
	 ...
	 * 
	 * @param sourceConditional    conditional node where "then"- and "else"-branch fork
	 * @param predecessorNodeThen  stores predecessor node of "then"-branch
	 * @param predecessorNodeElse  stores predecessor node of "else"-branch
	 * @param target   stores the original target node of "else"-branch.. this node joins "then"- and "else"-branch
	 * @param scg  original SCG
	 */
	def private void transformConditionalNodes(Conditional sourceConditional, Node predecessorNodeThen,
		...
		else if (predecessorNodeThen instanceof Assignment) {
		...
		if (thisNode instanceof Conditional) {
			...
			// Make sure this assignment is really relevant. Meaning multiple times assigned to.
	 ...
	 * 
	 * @param assignmentNode   assignment node which is inspected
	 * @param scg  original SCG
	 */
	def private transformAssignmentNodes(Assignment assignmentNode, SCGraph scg) {
		val name = assignmentNode.valuedObject.name
		val type = assignmentNode.valuedObject.type

		if (ssaMap.containsKey(name)) {
            
            //Update the version number of this variable.
			transformExpressions(assignmentNode.assignment)
			val ssaVersionNumber = ssaMap.get(name)
			ssaMap.replace(name, ssaVersionNumber, ssaVersionNumber + 1)
			...
			if ((outputOccurenceCounter.get(name) == null) ||
				...
				val dec = createDeclaration(type)
...
//				}
				...
				assignmentNode.valuedObject = vo
			...
			val expr = assignmentNode.assignment
			assignmentNode.assignment = transformExpressions(expr)
		}
	 ...
	 */
	def private transformExpressions(Expression expression) {
	 ...
	 * 
	 */
	def private filterVariablesWithMultipleWrites(List<Assignment> assignments) {
		for (a : assignments) {
			val name = a.valuedObject.name
			// gX and _condgx are unique
			
			
			// TODO: //if (!(name.startsWith("g") || (name.startsWith("_cond")))) {
