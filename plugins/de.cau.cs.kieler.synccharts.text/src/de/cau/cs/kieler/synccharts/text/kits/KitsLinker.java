 ...
 * It doesn't create proxies but setups cross references and their
 * EOpposites immediately.
 *   
 *  @author chsch
 */
...
public class KitsLinker extends AbstractCleaningLinker {
	...
	protected void doLinkModel(EObject model,
				...
				for (AbstractNode childNode : node.getChildren()) {
					...
					if (childNode.getGrammarElement() instanceof CrossReference) {
						...
						EReference eRef = GrammarUtil.getReference(
								(CrossReference) childNode
										.getGrammarElement(), obj.eClass());
						
						/* compute the relative path from node to childnode */
						uriFragment = getRelativePath("", node,	childNode);

						/* ask the linking service (with hooked scope provider for
						 * the addressed semantic element; list is empty or singleton */
						linkedObjects = this.linkingService.getLinkedObjects(
								obj, eRef, getNode(node, uriFragment));
	...
	/** computes relative address from currentNode to otherNode.
	 * copied from {@link org.eclipse.xtext.linking.lazy.LazyURIEncoder}
	 * (renamed parameters making them more comprehensible)
	 * @param result initial fragmentURI
	 * @param currentNode current semantic element parse node
	 * @param otherNode other node to compute the address from current node to
	 * @return
	 */	
	private String getRelativePath(String result, AbstractNode currentNode, AbstractNode otherNode) {
		...
		if (EcoreUtil.isAncestor(currentNode, otherNode)) {
			...
			result = result.concat("/").concat(Integer.toString(otherNode.getParent().getChildren().indexOf(otherNode)));
		} else {
	...
	/** reveals a parse tree node relative from 'node'.
	 * copied from {@link org.eclipse.xtext.linking.lazy.LazyURIEncoder}
	 * (renamed parameters making them more comprehensible)
	 * @param node current semantic element parse node
	 * @param path relative path to addressed node
