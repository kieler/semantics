...
public class KitsScopeProvider extends AbstractDeclarativeScopeProvider {
    ...
    /**
     * A implementation of scoping for transitions' targets. Won't be called directly but via
     * reflection by the Xtext runtime.
     * 
     * @param trans
     * @param reference
     * @return
    ...
    public IScope getScopeOld(EObject context, EReference reference) {
        ...
        if (context instanceof Transition) {
            List<State> states = ((Region) ((Transition) context).eContainer().eContainer())
                    .getStates();
            List<IEObjectDescription> stateDescriptions = new LinkedList<IEObjectDescription>();
            for (State state : states) {
                stateDescriptions.add(new EObjectDescription(state.getLabel(), state,
                        Collections.EMPTY_MAP));
            }
    ...
    private Iterable<IEObjectDescription> getElements(EObject parent) {
        ...
        if (parent != null && parent instanceof Region) {
            ...
            for (Signal signal : signals) {
                elements.add(new EObjectDescription(signal.getName(), signal,
                        (Map<String, String>) Collections.EMPTY_MAP));
            }
            List<Variable> variables = ((Region) parent).getVariables();
            for (Variable variable : variables) {
                elements.add(new EObjectDescription(variable.getName(), variable,
                        (Map<String, String>) Collections.EMPTY_MAP));
            }
