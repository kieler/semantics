...
public abstract class GenParser extends AbstractXtextParser implements IXtextParser {
	...
	private kittyLexer lexer;
	private kittyParser parser;

	public GenParser(InputStream in) {
		this.reader = new InputStreamReader(in);
	}

	public kittyLexer getLexer() {
		if (lexer == null) {
			try {
				this.lexer = new kittyLexer(new ANTLRReaderStream(reader));
			} catch (IOException e) {
				handleError(e);
			}
		}
	...
	public kittyParser getParser() {
		if (parser == null) {
			parser = new kittyParser(new CommonTokenStream(getLexer()));
		}
		return parser;
	}
	...
	protected void internalLink() {
		...
		try {
			...
			XtendFacade facade = XtendFacade.create(ctx, "de::cau::cs::kieler::Linking");
	...
	public Issues doCheck() {
		...
		try {
			...
			String[] checkFiles = new String[]{"de::cau::cs::kieler::GenChecks","de::cau::cs::kieler::Checks"};
