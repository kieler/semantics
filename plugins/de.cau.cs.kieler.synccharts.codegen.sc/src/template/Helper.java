...
public class Helper {
	...
	/**
	 * Computes the priority of the state. This is similar to the thread
	 * priority, if the state represents a new thread
	 * 
	 * @param state
	...
	public static final int computeChangePriority(final State state) {
		...
		for (String s : childThreadList) {
			if (threadListSorted.contains(s)) {
				intList.add(mainThreadPriority - 1
						- threadListSorted.indexOf(s));
			}
		}
	...
	private static List<State> sortStateControlFlowHelp(
			final List<State> sortedStates, final State state) {
		sortedStates.add(state);
		for (Transition transition : state.getOutgoingTransitions()) {
			if (!sortedStates.contains(transition.getTargetState())) {
				sortStateControlFlowHelp(sortedStates, transition
						.getTargetState());
			}
		}
		return sortedStates;
				...
				for (State innerState : region.getInnerStates()) {
					if (state.getParentRegion().getParentState() != null) {
						if (innerState.isIsInitial()) {
							// unterscheidung zwischen weak und strong bei
							// parent und child
							...
							depOne = builtDependency(parentStateTupelWeak,
									childStateTupel, NORMAL_EDGE);
							...
							Tuple<State, Integer> parentStateTupelStrong = new Tuple<State, Integer>(
									state, STRONG_STATE);
							depTwo = builtDependency(childStateTupel,
									parentStateTupelStrong, NORMAL_EDGE);
	...
	private static void fillSortedThreadList() {
		...
		LinkedList<Integer> threadListTopologicalSorted = dependencyGraph
				.topologicalSort();
		System.out.println("threadListTopologicalSorted: "
				+ threadListTopologicalSorted);
