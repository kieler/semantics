...
/**
 * The Class JSONObjectDataComponent. Implementation for the extension point
 * JSONObjectDataComponent that implements the interface
 * {@link IJSONObjectDataComponent}.
 * 
...
public abstract class JSONObjectSimulationDataComponent extends
	...
	private final IStatus model2ModelTransform(ProgressMonitorAdapter monitor)
		...
		return new Status(IStatus.OK, DataComponentPlugin.PLUGIN_ID,
	...
	public final JSONObject step(JSONObject jSONObject)
			throws KiemExecutionException {
		try {
			long newModelTimeStamp = this
					.getInputModelEObject(this.modelEditor).eResource()
					.getTimeStamp();
			...
			if (((newModelTimeStamp != modelTimeStamp) || modelEditor.isDirty())
				...
				throw new KiemExecutionException(
						"The simulated model changed since the simulation "
								...
								+ "simulate the changed version of your model.",
						false, null);
			}
		} catch (Exception e) {
	...
	/**
	 * Check the model validation. Should return true if the model is valid,
	 * false otherwise. <BR>
	 * <BR>
	 * Example:<BR>
	...
	private boolean askForModelError() throws KiemInitializationException {
		...
		if ((askForModelErrorReturnValue == 2)
				|| (askForModelErrorReturnValue == 1)) {
			// do not show a message, just abort execution initialization
			throw new KiemInitializationException(
					"User aborted model simulation/execution.", true, null,
					true);
		}
	...
	protected URI getInputModelAsURI() {
		...
		if (file != null) {
			URI uri = URI.createURI("file://"
					+ file.replaceAll(" ", "%20").replace("\\", "/"));// .replaceAll(" ",
																		// "%20"));
			return uri;
		}
		return null;
	...
	protected EObject getInputModelEObject(IEditorPart editorPart) {
		...
		if (editorPart instanceof DiagramEditor) {
			model = GmfModelingUtil
					.getModelFromGmfEditor((DiagramEditor) editorPart);
		} else if (editorPart instanceof XtextEditor) {
			model = XtextModelingUtil
					.getModelFromXtextEditor((XtextEditor) editorPart);
		}
		return model;
	}
	...
	protected View getNotationElement(IEditorPart diagramEditor) {
		if (diagramEditor instanceof DiagramEditor) {
			View notationElement = ((View) ((DiagramEditor) diagramEditor)
	...
	private final boolean performModelTransformation()
			throws KiemInitializationException {
	...
	public final KiemProperty[] provideProperties() {
		...
		if (properties == null) {
			...
			returnProperties[0] = new KiemProperty("Model Editor",
	...
	private boolean askForModelIsSaved() throws KiemPropertyException {
		...
		if (askForModelIsSavedReturnValue == 2) {
			...
			throw new KiemPropertyException(
					"User aborted model simulation/execution.", null, true);
		}
	 ...
	 * 
	 * @see
	 * de.cau.cs.kieler.sim.kiem.extension.DataComponent#testProperties(de.cau
	 * .cs.kieler.sim.kiem .data.KiemProperty[])
	 */
	@Override
	public void checkProperties(KiemProperty[] properties)
		...
		if (modelEditor.isDirty()) {
			try {
				...
				throw new KiemPropertyException(
						"There are unsaved changes of the model opened "
			...
			if (eObject != null) {
				...
				// if dialog cannot be opened, throw error
				throw new KiemPropertyException(
						"The selected Editor cannot be used for simulation/execution!");
			}
		}
	}
	...
	protected IEditorPart getEditor(String kiemEditorProperty) {
		...
		if (tokenizer.hasMoreTokens()) {
			...
			IEditorReference[] editorRefs = getActivePage()
					.getEditorReferences();
