...
public class JSONDataPool {
    ...
    /**
     * This is the maximum number of consecutive new entries that will be kept in history for a
     * delta update meaning that between two consecutive calls of getDeltaData there should not be
     * more than DATA_DELTA_POOL_HISTORY_MAX new entries been produced (regardless of any steps!)
     * note that the dataDeltePool is needed for Delta Observer and for history steps only.
     */
    ...
    /**
     * Gets the most current data. This is the most efficient method to call since it just returns
     * the current data pool contents. But be aware of the fact that the returned JSON object may
     * have hundreds of values.
     * 
     * @param filterKeys
     *            the filter keys, optional, may be null
     * @param deltaPoolIndexTo
     ...
     *             a JSON exception
     */
    public JSONObject getData(final String[] filterKeys, 
                             final long deltaPoolIndexTo) throws JSONException {
     ...
     *             the JSON exception
     */
    public JSONObject getData(final String[] filterKeys) throws JSONException {
        JSONObject returnObject = null;
        if (filterKeys == null) {
            // all data
            // return a copy of all data
            returnObject = new JSONObject(this.dataPool.toString());
        } else {
            // filtered data
    ...
    /**
     * Gets the pool start index from a provided deltaIndex. if the deltaIndex is FAR TOO old, then
     * it will return <B>ALL</B> values accumulated within index 0 of the delta list.
     * 
     * @param deltaIndex
     *            the delta index
     * 
     ...
     */
    private int getPoolIndex(final long deltaIndex) {
    ...
    /**
     * Gets the delta data between (since) the deltaPoolIndexFrom and the deltaPoolIndexTo or the
     * current index if the latter is not provided. It optionally filters the results if filterKeys
     * is not null.
     * 
     * @param filterKeys
     *            the filter keys, optional, may be null
     * @param deltaPoolIndexFrom
     ...
     *             the JSON exception
     */
    public JSONObject getDeltaData(final String[] filterKeys, final long deltaPoolIndexFrom)
            throws JSONException {
        return getDeltaData(filterKeys, deltaPoolIndexFrom, this.getPoolCounter());
    }
    ...
    /**
     * Puts a new JSONObject into the data pool. It will also put the item into the delta pool list.
     * If the list gets too large (according to DATA_DELTA_POOL_HISTORY_MAX) then we accumulate the
     * history older than that into the object with the index 0. This allows us to play back up to
     * DATA_DELTA_POOL_HISTORY_MAX data producers (*NOT* ticks).
     * 
     * @param newData
     ...
     *             a JSON exception
     */
    public synchronized void putData(final JSONObject newData) throws JSONException {
