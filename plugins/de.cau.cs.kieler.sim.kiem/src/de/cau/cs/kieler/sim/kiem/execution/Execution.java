...
/**
 * The Class Execution. This is the base class for the whole execution. It
 * creates and manages the worker threads for pure observer and pure producer
 * components. It also implements the scheduling between all components.
 * Additionally it manages a timeout component that checks whether a
 * component-method exceeds the maximum timeout and then terminates all
 * executions w/ an error log message.
 * 
...
public class Execution extends Job {
	...
	private static final String HEAPERRORMESSAGE = "Out of heap memory. Cannot create dynamic execution threads. You're suggested to restart the program.";
	...
	/**
	 * The Constant SECOND_WAITTIMEOUT indicates how long to sleep until changes
	 * in aimed step durations are guaranteed to be processed.
	 */
	...
	/**
	 * The step to perform can be NO_STEPS in pause mode or INFINITY_STEPS in
	 * run mode or FORWARD_STEP / BACKWARD_STEP.
	 */
	private long steps;
	...
	/**
	 * The step counter max. By default this is equal to stepCounter. In case of
	 * history steps: stepCount <= stepCounterMax.
	 */
	 ...
	 * but minimally a timeout of Execution.SECOND_WAITTIMEOUT. In this case the
	 * plugins content providers are notified of the default timeout value.
	 * 
	 * author: soh
	 * 
	 * @return the timeout
	...
	private int getTimeout() {
		try {
			...
			if (providedTimeout != null) {
				...
				int aimedStepDurationTmp = KiemPlugin.getDefault()
						.getAimedStepDuration();
				int returnValue = Execution.TIMEOUTMULTIPLICITY
						* aimedStepDurationTmp;
	...
	public Execution(
		...
		this.timeout.start();
		observerExecutionArray = new ObserverExecution[this.dataComponentWrapperList
				.size()];
		producerExecutionArray = new ProducerExecution[this.dataComponentWrapperList
				.size()];
		...
		try {
			...
			KiemPlugin.getDefault().showError(HEAPERRORMESSAGE, KiemPlugin.PLUGIN_ID, new Exception(e), false);
			errorCleanUp();
	...
	/**
	 * Gets the data pool. Be careful with using the DataPool after starting the
	 * thread!
	 * 
	 ...
	 * current step (
	 * 
	 * @return the maximum step {@link #getSteps()}), unless the current step is
	 *         a history step.
	 */
	 ...
	 * this is a step number in the future then run the execution until the
	 * stepCounter has reached this number. If this is a step number back in the
	 * history, then just jump to it. If this is a step number somewhere later
	 ...
	 * or a calling master component, if the currently processed step is not yet
	 * done, this method returns false and no processes another step. Otherwise,
	 * if the last step was already processed, the method will schedule the next
	 * step but will return asynchronously. It will first synchronously inform
	 * all components about the step command.
	 * 
	 * @return true, if step is being processed
	 */
	...
	/**
	 * Make a forward execution step. Because we do not want to block the GUI or
	 * a calling master component, if the currently processed step is not yet
	 * done, this method returns false and no processes another step. Otherwise,
	 * if the last step was already processed, the method will schedule the next
	 * step but will return asynchronously. It will first synchronously inform
	 * all components about the step command.
	...
	/**
	 * Pauses the execution. The pauseCommand flag is set to true to immediately
	 * pause the execution. Before the steps are being set to {@link #NO_STEPS}
	 * the DataComponents are informed about the pause command.
	 */
	 ...
	 * are being set to infinity, which is interpreted by the
	 * {@link de.cau.cs.kieler.sim.kiem.execution.Execution#run(IProgressMonitor)}
	 * method. Before that is done the DataComponents are informed about the run
	 * command.
	 */
	 ...
	 * that no further step is made. The data components are then being informed
	 * about the stop command. For safety reasons we again set the stop flag in
	 * synchronized monitor of this object and then stop all worker threads that
	 * where started. The latter includes the timeout thread.
	 */
	...
	/**
	 * Aborts the execution. In this case the stop flag is immediately set so
	 * that no further step is made. The data components are then being informed
	 * about the stop command. For safety reasons we again set the stop flag in
	 * synchronized monitor of this object and then stop all worker threads that
	 * where started. The latter includes the timeout thread.
	 ...
	 * backward). This is *NOT* necessary equivalent to the term "!isRunning()"!
	 * 
	...
	/**
	 * Terminates the execution in case of an error. This will also (try) to
	 * terminate all created helper threads. They may not finish immediately and
	 * in case of a deadlock they won't do so ever, but the user will get a
	 * response. This function is called by the TimeoutThread. Hence the call of
	 * this method should only happen during the development of a new component.
	 */
	 ...
	 * be used if the wrapupComponents is called by the handleComponentError to
	 * not get any recusive call!
	 * 
	 * @param quietmode
	...
	public synchronized void wrapupComponents(final boolean quietmode) {
		for (int c = 0; c < this.dataComponentWrapperList.size(); c++) {
			DataComponentWrapper dataComponentWrapper = dataComponentWrapperList
					.get(c);
			if (dataComponentWrapper.isEnabled()) {
				timeout.timeout(getTimeout(), "wrapup", dataComponentWrapper,
						this);
				try {
	 ...
	 * history flag is set (hence the current step is a history step) then this
	 * component will get the full data. Otherwise the isDeltaObserver() method
	 * is inspected to find out if the DataComponent only wants the delta data
	 * between the last call.
	 * 
	...
	private JSONObject getInputData(
			...
			throws JSONException {
		timeout.timeout(getTimeout(), "provideFilterKeys",
				dataComponentWrapper, this);
		...
		if (this.isHistoryStep()) {
			...
			long oldPoolCounter = dataComponentWrapper.getPoolIndex(this
					.getSteps());
			...
			if (dataComponentWrapper.isDeltaObserver()) {
				oldData = this.dataPool.getDeltaData(filterKeys,
						lastPoolCounter);
			} else {
			...
			dataComponentWrapper.addPoolIndex(this.dataPool.getPoolCounter(),
					this.getSteps());
		}
	...
	private boolean makeStepObserverProducer(
		...
		if (dataComponentWrapper.isJSON()) {
			...
			try {
				newData = ((JSONObjectDataComponent) dataComponentWrapper
			...
			String newData = null;
			try {
				newData = ((JSONStringDataComponent) dataComponentWrapper
			...
			while (!this.stop) {
				...
				synchronized (this) {
					...
					if (this.stepCounter == this.stepToPause) {
						...
						// update the GUI
						eventManager.notify(new KiemEvent(
								KiemEvent.VIEW_REFRESH));
					}
					...
					if ((steps == INFINITY_STEPS) || (steps != NO_STEPS)) {
						...
						if ((steps == FORWARD_STEP)
								|| (steps == INFINITY_STEPS)) {
							if (this.stepCounter == this.stepCounterMax) {
						...
						// notify about steps
						for (int c = 0; c < this.dataComponentWrapperList
								.size(); c++) {
							KiemEvent infoEvent = new KiemEvent(
									KiemEvent.STEP_INFO, new Pair<Long, Long>(
											this.stepCounter,
											this.stepCounterMax));
							if (eventManager != null) {
						...
						if (!this.isHistoryStep()) {
							for (int c = 0; c < this.dataComponentWrapperList
								...
								timeout.timeout(getTimeout(),
										"isEnabled, isProducer, isObserver",
										dataComponentWrapper, this);
								if (dataComponentWrapper.isEnabled()
						...
						for (int c = 0; c < this.dataComponentWrapperList
							...
							if (dataComponentWrapper.isProducerObserver()) {
								...
								try {
									...
									if (!this.isHistoryStep()) {
										dataComponentWrapper
												.setDeltaIndex(this.dataPool
														.getPoolCounter());
									}
								} catch (Exception e) {
									if (!stop) {
										KiemPlugin.getDefault().showWarning(
												null, KiemPlugin.PLUGIN_ID, e,
												false);
									}
								}
								// System.out.println(dataComponentWrapper.getName()
								...
								timeout.timeout(getTimeout(), "step (reap)",
										dataComponentWrapper, this);
								try {
									// System.out.println(c + ") "
									// +dataComponentWrapper.getName() +
								...
								// " (Pure Producer) done");
								try {
									JSONObject newData = producerExecutionArray[c]
											.getData();
									if (newData != null) {
										this.dataPool.putData(newData);
									}
				...
				if (steps == INFINITY_STEPS) {
					int timeToDelay = this.aimedStepDuration
							- this.stepDuration;
					if (timeToDelay > 0) {
	 ...
	 *            the plug-in id
	 * @param e
	 *            the original Exception {@link ProducerExecution} or the
	 *            {@link TimeoutThread}.
	 */
