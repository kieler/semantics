...
//	    depGraph = prog.getDepGraph();
	...
	/*for (Entry<String, Expression> e : prog.getEqs().entrySet()) {
	    int id = cluster.getID(e.getKey());
	    if (id != 0) {
		cores.get(id - 1).addEq(e.getKey(), e.getValue());
	    }
	}
	...
	/*for (Entry<String, Type> i : prog.getInputs().entrySet()) {
	    res += "INPUT " + i.getKey() + "\n";
	}
	for (Entry<String, Type> i : prog.getOutputs().entrySet()) {
	    res += "OUTPUT " + i.getKey() + "\n";
	}
	res += "\n";
