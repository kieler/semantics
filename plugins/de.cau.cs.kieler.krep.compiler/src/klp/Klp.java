...
public class Klp extends Program {
    ...
    /**
     * Number of input output regs
     */
    public final static int N_IO = 4;

    /**
     * Number of full local regs
     */
    public final static int N_LOCAL = 4;

    /**
     * Number of temporary regs
     */
    public final static int N_TEMP = 4;
     ...
     */
    public LinkedList<Instruction> compile(boolean useClocks, String scope) {
    ...
    public LinkedList<Instruction> compileInit(boolean useClocks, String scope, boolean setInputs,
	    boolean setOutputs, int prioOffset) {
	...
	for (Equation e : eqs) {
		...
		res.add(new InitReg(v.getName(), v.getKind(), v.getName()
			+ scope, useClocks?clock:null));
