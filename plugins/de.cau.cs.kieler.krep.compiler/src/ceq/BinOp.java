...
public class BinOp extends Expression {
	...
	if (v1 != null && v2 != null) {
	    instr.add(new klp.instructions.BinOp(r, new Read(v1), new Read(v2),
		    op));
	} else if (v1 != null && c2 != null) {
	    instr.add(new klp.instructions.IBinOp(r, new Read(v1), c2.getVal(),
		    op));
	} else if (c1 != null && v2 != null) {
	    instr.add(new klp.instructions.IMov(r, c1.getVal()));
	    instr.add(new klp.instructions.BinOp(r, new Read(r, false),
		    new Read(v2), op));
	} else if (v1 != null) {
	    instr.addAll(e2.toKlp(r));
	    instr.add(new klp.instructions.BinOp(r, new Read(v1), new Read(r,
		    false), op));
	} else if (v2 != null) {
	    ...
	    instr.addAll(e1.toKlp(r));
	    instr.add(new klp.instructions.IBinOp(r, new Read(r, false), c2
		    .getVal(), op));
	} else {
	    ...
	    instr.add(new klp.instructions.BinOp(r, new Read(temp1), new Read(temp2),
		    op));
	    Variable.destroyTemp(name);
	    Variable.destroyTemp(name);
