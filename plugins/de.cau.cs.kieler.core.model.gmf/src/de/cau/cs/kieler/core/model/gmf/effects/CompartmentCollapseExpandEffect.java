...
public class CompartmentCollapseExpandEffect extends AbstractEffect {
	...
	/**
	 * hierarchy level. 0 means only exactly the given EditPart. TODO implement
	 * compartment levels
	 */
	...
	/**
	 * The compartment level gives the hierarchy to which to search for
	 * compartments to doCollapse.
	 * 
	 * @param editor
	 *            the DiagramEditor containing the EObject
	 * @param node
	 ...
	 *            hierarchy level. 0 means only exactly the given EditPart. Not
	 *            implemented.
	 * @param collapse
	 *            true if collapsing, false if expanding
	 */
	public CompartmentCollapseExpandEffect(final IWorkbenchPart editor,
			final EObject node, final EStructuralFeature thefeatureToCollapse,
			final int theCompartmentLevel, final boolean collapse) {
		...
		this.bridge = GraphicalFrameworkService.getInstance().getBridge(
				targetEditor);
	}
	...
	/**
	 * Extracted the initialization into extra method that is NOT called in the
	 * constructor, because it accesses the EditParts (bridge.getEditPart),
	 * which runs in the UI thread. This could cause deadlocks if called outside
	 * the EffectsWorker Thread (in which the execute method gets called).
	 */
	private void init() {
...
//		}
		...
		for (IResizableCompartmentEditPart targetEditPart : targetEditParts) {
			if (targetEditPart != null
				...
				if (f.isExpanded() == collapse) {
					...
					if (collapse) {
						f.setCollapsed();
						n = new ENotificationImpl((InternalEObject) targetNode,
								Notification.SET, -1, Boolean.FALSE,
								Boolean.TRUE);
					} else {
						f.setExpanded();
						n = new ENotificationImpl((InternalEObject) targetNode,
								Notification.SET, -1, Boolean.TRUE,
								Boolean.FALSE);
					}
					changed = true;
					EditPart part = targetEditPart.getParent();
					if (n != null && part instanceof IAdvancedRenderingEditPart) {
						((IAdvancedRenderingEditPart) part)
								.handleNotificationEvent(n);
					}
				}
			}
		}
	...
	private Command getCollapseCommand(final boolean collapse) {
		...
		for (IResizableCompartmentEditPart targetEditPart : this.targetEditParts) {
			...
			SetPropertyCommand spc = new SetPropertyCommand(
					targetEditPart.getEditingDomain(),
					new EObjectAdapter(style),
					Properties.ID_COLLAPSED,
	...
	/**
	 * Determines whether the last call to execute() or undo() actually
	 * performed any changes.
	 * 
	...
	private boolean isCollapsed() {
		...
		for (IResizableCompartmentEditPart targetEditPart : targetEditParts) {
			if (targetEditPart != null
					&& targetEditPart.getModel() instanceof DrawerStyle) {
				// FIXME: This is specific to GMF notation model
				allCollapsed &= ((DrawerStyle) targetEditPart.getModel())
						.isCollapsed();
			} else {
