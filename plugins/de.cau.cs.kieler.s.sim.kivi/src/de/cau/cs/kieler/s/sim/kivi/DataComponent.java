...
public class DataComponent extends JSONObjectSimulationDataComponent {
	...
	/** The S editor. */
	private XtextEditor sEditor;

	/** The S semantic resource editor. */
	Resource semanticResource;

	/** The S program. */
	private Program sProgram;
	...
	/**
	 * The highlight background color is used internally and set NULL for
	 * original.
	 */
	...
	void refreshEObjectMap() {
		eObjectMap.clear();
		refreshEObjectMap(this.sProgram);
	}
	...
	public JSONObject doStep(JSONObject jSONObject)
		...
		try {
			...
			new KiemExecutionException(
					"fail silent recovery from user editor change failed.",
					false, e);
		}
		...
		for (EObject statement : newActiveErrorStatements) {
			...
			if (!oldActiveErrorStatements.contains(statement)
				|| 
				oldActiveStatements.contains(statement) && newErrorStatements.contains(statement)
				||
				oldErrorStatements.contains(statement) && newActiveStatements.contains(statement)) 
				{
				try {
	...
	private void setXtextSelection(EObject semanticElement,
			RGB specificBackgroundColor) throws KiemInitializationException {
	...
	/**
	 * Fail silent recovery from user editor change. If the user edits text in
	 * the editor during visualization, the highlighting is invalidated but the
	 * next time he clicks on step (and has focused the right editor) the
	 * refreshed editor input is used for visualization.
	 * 
	 * @throws KiemInitializationException
	 *             the kiem initialization exception
	 */
	private boolean failSilentRecoveryFromUserEditorChange(
			LinkedList<EObject> activeStatements)
			throws KiemInitializationException {
			...
			Display.getDefault().asyncExec(new Runnable() {
				public void run() {
					...
					xtextEditor.getInternalSourceViewer().setRangeIndication(
							offset, length, true);
					xtextEditor.getInternalSourceViewer().revealRange(offset,
							length);
					...
					if (specificBackgroundColor != null) {
						...
						StyleRange backupStyleRange = xtextEditor
						...
						StyleRange recoverStyleRange = recoverStyleRangeMap
								.get(offset);
						if (recoverStyleRange != null) {
							styleRange = new StyleRange(offset, length,
									recoverStyleRange.foreground,
									recoverStyleRange.background,
	 ...
	 * @throws KiemInitializationException
	 */
	private XtextEditor getSEditor() throws KiemInitializationException {
		...
		throw new KiemInitializationException(
				"No opened S editor found for simulation visualization.",
				true, null);
	}
	...
	public KiemProperty[] doProvideProperties() {
		...
		properties[1] = new KiemProperty("Error Statement Name",
				"errorStatement");
	 ...
	 * @param sEditor
	 *            the s editor
	 * @return the s program
	 */
	private Program getSProgram(XtextEditor sEditor) {
		...
		result = sEditor.getDocument().readOnly(work);
