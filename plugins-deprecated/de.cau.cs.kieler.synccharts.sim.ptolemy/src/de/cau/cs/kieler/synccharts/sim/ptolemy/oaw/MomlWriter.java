 ...
 * MOML files can be considered to be the XML representation of Ptolemy models and for example can
...
public class MomlWriter extends AbstractEMFWorkflowComponent {
     ...
     * 
     * @param b
     *            the new multiple resources in case of list
     */
    ...
    // *
    // * @param option_schema_location_implementation the new oPTIO n_ schem a_ locatio n_
    // implementation
    // */
    // public void setOPTION_SCHEMA_LOCATION_IMPLEMENTATION(final boolean
    ...
    public void invokeInternal(final WorkflowContext ctx, final ProgressMonitor monitor,
        ...
        if (!((slotContent instanceof Collection<?>) || (slotContent instanceof EObject))) {
            issues.addError(this, "slot '" + getModelSlot()
                    + "' neither contains an EList nor an EObject", slotContent, null, null);
            return;
        }
            ...
            if (cloneSlotContents) {
                if (sc.eResource() == null) {
                    issues.addWarning(
                            ...
                            "model in slot '"
                                    + getModelSlot()
                                    + "' is not yet associated with a resource;" 
                                    + " cloning it is most likely an error!");
                } else {
                ...
                if (sc.eResource() != null) {
                    issues.addWarning(
                            ...
                            "the element in slot '"
                                    + getModelSlot()
                                    + "' is already contained in a resource and" 
                                    + " will be taken out of that resource!");
                }
            }
        }
        ...
        if (!multipleResourcesInCaseOfList) {
            ...
            final XMLResource r1 = (XMLResource) resourceSet
                    .createResource(URI.createURI(getUri()));
            try {
            ...
            if (slotContent instanceof Collection<?>) {
                ...
                for (final Object object : coll) {
                    ...
                    // final Resource r =
                    // getResourceSet().createResource(URI.createURI(createResourceName(eo)));
                    final XMLResource r = (XMLResource) resourceSet.createResource(URI
                            .createURI(createResourceName(eo)));
    ...
    private void write(final XMLResource r) {
        try {
            ...
            r.setDoctypeInfo("-//UC Berkeley//DTD MoML 1//EN",
            ...
            throw new WorkflowInterruptedException("Problems writing moml file to " + getUri()
                    + " : " + e.getMessage());
        }
    }
